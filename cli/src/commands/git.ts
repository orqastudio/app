...
function gitSilent(args: string, root: string): string | null {
	try {
...
function classifyFile(filePath: string, components: readonly string[]): string {
	...
	// Governance files
	if (filePath.startsWith(".orqa/")) return ".orqa";
...
interface FileChange {
	...
	worktree: string; // Y column from porcelain
	filePath: string;
}

function parseStatusPorcelain(output: string): FileChange[] {
...
async function cmdStatus(root: string): Promise<void> {
	...
	const nonComponentEntries = [...grouped.keys()].filter(
		(k) => k === "(root)" || k === ".orqa",
	).length;
...
async function cmdPr(root: string, args: string[]): Promise<void> {
	...
	try {
		const result = execSync(
			`curl -s -X POST "${apiBase}/pulls" -H "Content-Type: application/json" -d '${payload.replace(/'/g, "'\\''")}'`,
 ...
 */
function deriveApiBase(remoteUrl: string): string | null {
	// HTTP(S) URL
...
async function cmdAudit(root: string): Promise<void> {
	...
	for (const remote of remotes) {
		...
		if (localRef === remoteRef) {
			...
			// Check ahead/behind
			const aheadBehind = gitSilent(
				`rev-list --left-right --count main...${remote.name}/main`,
				root,
			);
			if (aheadBehind) {
 ...
 */
export async function runGitCommand(args: string[]): Promise<void> {
