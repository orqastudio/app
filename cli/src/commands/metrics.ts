...
function parseOptions(args: string[]): MetricsOptions {
...
function printSessionSummary(sessions: SessionMetrics[]): void {
...
function printAgentSummary(agents: AgentMetrics[]): void {
	if (agents.length === 0) {
	...
	for (const agent of top) {
		...
		console.log(
			`  ${agent.agentId.substring(0, 12)}  ${agent.role.padEnd(14)} ${formatTokens(total).padStart(8)}  ${formatCost(cost).padStart(8)}  ${agent.model}  ctx:${formatPercent(agent.contextUtilization)}`,
		);
	}
	console.log();
}

function printTrends(projectRoot: string, periodDays: number): void {
