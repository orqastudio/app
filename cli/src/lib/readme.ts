 ...
 * @returns Array of audit results, one per audited directory.
 */
export function auditReadmes(projectRoot: string): ReadmeAuditResult[] {
	...
	for (const category of ["libs", "plugins", "connectors", "sidecars", "tools", "registry"]) {
 ...
 * @param opts - Template generation options.
 * @param opts.name - The package name (e.g. "@orqastudio/my-lib").
 * @param opts.displayName - Human-readable display name.
 * @param opts.description - One-sentence package description.
 * @param opts.category - Package category for badge styling.
 * @param opts.license - SPDX license identifier.
 * @param opts.languages - Detected language names for badges.
 * @returns The generated README markdown string.
 */
