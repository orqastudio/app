...
import { countOnDemandEntries, generateOnDemandPreamble, retrieveKnowledge, } from "../lib/knowledge-retrieval.js";
...
assert(registry.knowledge.length >= 0, `Registry has ${registry.knowledge.length} knowledge entries`);
assert(registry.sections.length >= 0, `Registry has ${registry.sections.length} prompt sections`);
assert(typeof registry.built_at === "string", "Registry has a built_at timestamp");
// ---------------------------------------------------------------------------
...
assert(result.prompt.includes("<role>implementer</role>"), "Prompt contains role tag");
assert(result.prompt.includes("<task-context"), "Prompt contains task-context section");
assert(result.prompt.includes("<task-description>"), "Prompt contains task-description");
assert(result.prompt.includes("Fix the login validation bug"), "Prompt contains the task description text");
assert(result.prompt.includes("<acceptance-criteria>"), "Prompt contains acceptance criteria");
// Budget compliance
const budget = DEFAULT_TOKEN_BUDGETS["implementer"] ?? 2800;
assert(result.budget === budget, `Budget is ${budget} for implementer`);
assert(result.totalTokens <= budget * 1.1, // Allow 10% overshoot from XML overhead
...
for (const section of result.includedSections) {
    assert(typeof section.id === "string" && section.id.length > 0, `Section "${section.id}" has an id`);
    assert(typeof section.content === "string" && section.content.length > 0, `Section "${section.id}" has content`);
    assert(typeof section.tokens === "number" && section.tokens > 0, `Section "${section.id}" has token count (${section.tokens})`);
}
...
assert(typeof onDemandCount === "number", `On-demand entry count: ${onDemandCount}`);
const preamble = generateOnDemandPreamble(onDemandCount);
if (onDemandCount > 0) {
    assert(preamble.includes("on-demand-knowledge"), "Preamble contains on-demand-knowledge tag");
    assert(preamble.includes("search_semantic"), "Preamble references semantic search tool");
}
else {
...
assert(nonEmptyPreamble.includes("<on-demand-knowledge>"), "Positive count produces preamble with opening tag");
assert(nonEmptyPreamble.includes("</on-demand-knowledge>"), "Positive count produces preamble with closing tag");
// ---------------------------------------------------------------------------
...
const totalRetrievedTokens = budgetedRetrieved.reduce((sum, k) => sum + k.tokens, 0);
assert(totalRetrievedTokens <= 100, `Retrieved knowledge within budget: ${totalRetrievedTokens} <= 100`);
// ---------------------------------------------------------------------------
...
assert(noRegistryResult.errors[0].includes("registry"), "Error mentions registry");
// ---------------------------------------------------------------------------
// Summary
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=prompt-pipeline.integration.js.map