...
import { STATE_CATEGORIES, } from "@orqastudio/types";
 ...
 */
export function discoverWorkflows(projectRoot) {
    const workflows = [];
    const errors = [];
 ...
 */
export function discoverOverrides(projectRoot) {
    const overrides = [];
    const errors = [];
 ...
 */
export function applyOverrides(definition, overrides, metadata) {
    const errors = [];
    ...
    for (const { filePath, override } of applicable) {
        ...
        if (override.transitions) {
            for (const transition of override.transitions) {
                const isDuplicate = result.transitions.some((t) => JSON.stringify(t.from) === JSON.stringify(transition.from) &&
                    t.to === transition.to &&
 ...
 */
export function matchContributions(workflows) {
    ...
    for (const workflow of nonSkeletons) {
        ...
        const contributesTo = workflow.definition
            .contributes_to;
 ...
 */
export function mergeContributions(skeleton, contributions) {
    // Deep clone the skeleton definition
    const merged = JSON.parse(JSON.stringify(skeleton.definition));
    const metadata = {
    ...
    for (const point of points) {
        ...
        for (const contribution of pointContributions) {
            ...
            for (const transition of contribution.transitions) {
                ...
                const isDuplicate = merged.transitions.some((t) => JSON.stringify(t.from) === JSON.stringify(transition.from) &&
                    t.to === transition.to &&
                    t.event === transition.event);
                if (!isDuplicate) {
 ...
 */
export function validateResolvedWorkflow(definition, metadata) {
    const errors = [];
    // Required fields
    if (!definition.name)
    ...
    if (!definition.initial_state)
        errors.push("Missing required field: initial_state");
    ...
    const validCategories = new Set(STATE_CATEGORIES);
 ...
 */
function buildIdPatternForSchema(idPrefix) {
 ...
 */
function writeStageResolvedWorkflows(skeleton, contributions, projectRoot, outputDir) {
    ...
    for (const point of points) {
        ...
        for (const contrib of sorted) {
            ...
            for (const transition of contrib.transitions) {
                const isDupe = stageTransitions.some((t) => JSON.stringify(t.from) === JSON.stringify(transition.from) &&
                    t.to === transition.to &&
                    t.event === transition.event);
                if (!isDupe) {
 ...
 */
function buildArtifactTypeSummary(schema, pluginName) {
 ...
 */
function buildStateMapFromTransitions(statusTransitions) {
    const states = {};
 ...
 */
export function resolveAllWorkflows(projectRoot) {
    const result = {
        resolved: [],
        standalone: [],
        errors: [],
    ...
    for (const skeleton of skeletons) {
        ...
        if (hasStages) {
            try {
                ...
                result.errors.push(`Stage workflow writing failed for ${skeleton.definition.name}: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
    }
 ...
 */
function buildResolvedJson(definition, metadata) {
 ...
 * @param projectRoot - Absolute path to the project root.
 */
export function runWorkflowResolution(projectRoot) {
    ...
    for (const r of [...result.resolved, ...result.standalone]) {
        ...
        const parts = [];
        if (contribCount > 0)
            parts.push(`${contribCount} contribution(s)`);
        if (overrideCount > 0)
            parts.push(`${overrideCount} override(s)`);
