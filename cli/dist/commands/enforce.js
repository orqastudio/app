...
export async function runEnforceCommand(projectRoot, args) {
    ...
    const stagedRustPackages = stagedFiles !== null ? getStagedRustPackages(projectRoot, stagedFiles) : null;
    ...
    for (const engine of toRun) {
        ...
        if (stagedFiles !== null && engine.fileTypes.length > 0) {
            ...
            if (ignoreFileName) {
                ...
                if (relevant.length === 0)
                    continue; // All files ignored — skip engine.
            }
            if (action.files) {
                files = relevant;
            }
 ...
 */
function findCargoManifestDir(projectRoot, filePath) {
 ...
 */
function readCargoPackageName(manifestDir) {
    try {
 ...
 */
function getStagedRustPackages(projectRoot, stagedFiles) {
 ...
 */
function checkTauriFrontendDirs(projectRoot, packages) {
    let missing = false;
    for (const pkg of packages) {
        if (!TAURI_CRATE_NAMES.has(pkg))
            continue;
        ...
        if (frontendDir && !existsSync(join(projectRoot, frontendDir))) {
            ...
            console.error(`ERROR: ${pkg} requires a built frontend but ${frontendDir}/ does not exist.\n` +
 ...
 */
function buildScopedCargoArgs(baseArgs, packages) {
 ...
 */
function filterByIgnorePatterns(files, ignorePatterns) {
 ...
 */
function runBuiltinCheck(projectRoot, command, files) {
...
function checkComponentStories(root, stagedFiles) {
    ...
    for (const sub of dirs) {
        ...
        for (const entry of readdirSync(baseDir, { withFileTypes: true })) {
            ...
            if (stagedDirs && !stagedDirs.has(`${sub}/${entry.name}`))
                continue;
