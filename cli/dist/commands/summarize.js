 ...
 */
function generateSummary(title, body, description) {
    const parts = [];
 ...
 */
function extractPurpose(title, body, description) {
 ...
 */
function extractWhenToUse(body, description) {
 ...
 */
function summarizeFile(filePath, force) {
    ...
    const id = fm.id ?? "";
    const title = fm.title ?? path.basename(absPath, ".md");
 ...
 */
function findKnowledgeArtifacts(projectRoot) {
    const results = [];
    const dirs = [
 ...
 * @param projectRoot - Absolute path to the project root.
 */
function checkMissingSummaries(projectRoot) {
    ...
    for (const filePath of files) {
        ...
        if (!fm.summary) {
            ...
            const tokens = estimateTokens(fm.summary);
            if (tokens < SUMMARY_MIN_TOKENS) {
                console.log(`  SHORT    ${id}  ${title} (${tokens} tokens, target ${SUMMARY_MIN_TOKENS}-${SUMMARY_MAX_TOKENS})`);
                belowTarget++;
            }
            hasSummary++;
        }
 ...
 */
export async function runSummarizeCommand(args) {
