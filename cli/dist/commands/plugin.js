...
import { readContentManifest, writeContentManifest, installPluginDeps, buildPlugin, runLifecycleHook, processAggregatedFiles, } from "../lib/content-lifecycle.js";
 ...
 */
function resolvePluginDir(name, projectRoot) {
 ...
 */
function removeProjectJsonPlugin(projectRoot, name) {
    const projectJsonPath = path.join(projectRoot, ".orqa", "project.json");
    if (!fs.existsSync(projectJsonPath)) {
        return;
    }
...
async function cmdInstall(args) {
    ...
    const isFirstParty = absSource.startsWith(path.join(projectRoot, "plugins")) ||
 ...
 * @param projectRoot - Absolute path to the project root.
 */
async function cmdInstallFirstParty(pluginDir, projectRoot) {
