 ...
 * @returns Array of delivery levels with parent/child relationships.
 */
function loadDeliveryHierarchy() {
    const root = getRoot();
    const levels = [];
    for (const container of ["plugins", "connectors", "sidecars"]) {
        ...
        let entries;
        try {
            entries = fs.readdirSync(containerDir, { encoding: "utf-8" });
        }
        catch {
            continue;
        }
...
function printTree(results) {
     ...
     * @param parentNode - The parent node to print children for.
     * @param depth - Current indentation depth.
     */
    function printLevel(parentNode, depth) {
        ...
        for (const childLevel of childLevels) {
            const childNodes = (nodesByType.get(childLevel.type) ?? []).filter((n) => n.relationships.some((r) => r.target === parentNode.id && r.type === childLevel.parentRelationship));
    ...
    for (const level of hierarchy) {
        ...
        const orphans = nodes.filter((n) => !n.relationships.some((r) => r.type === level.parentRelationship && parentNodes.some((p) => p.id === r.target)));
        if (orphans.length > 0) {
...
function loadStatusCategories() {
    ...
    let entries;
    try {
        entries = fs.readdirSync(workflowsDir, { encoding: "utf-8" });
    }
    catch {
