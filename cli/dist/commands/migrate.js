...
import { runMigrateStorageIngestCommand, runMigrateStorageManifestCommand, } from "./migrate-storage.js";
const USAGE = `
 ...
 */
function loadMigrationMappings(projectRoot) {
    const workflowDir = join(projectRoot, ".orqa", "workflows");
    let entries;
    try {
        entries = readdirSync(workflowDir, { withFileTypes: true, encoding: "utf-8" });
    }
    catch {
        return [];
    }
    ...
    for (const entry of entries) {
        if (!entry.name.endsWith(".resolved.json"))
            continue;
 ...
 */
function computeChanges(projectRoot, mappings, typeFilter) {
    const artifacts = scanArtifacts(projectRoot);
    const changes = [];
    for (const mapping of mappings) {
        if (typeFilter && mapping.artifactType !== typeFilter)
            continue;
