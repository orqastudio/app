 ...
 * @param requires - The `requires` field value from the manifest.
 * @param extendsField - The `extends` field value from the manifest.
 * @returns Deduplicated array of dependency names.
 */
function mergePluginDeps(requires, extendsField) {
    const deps = new Set();
 ...
 * @param plugins - Loaded plugin list to check.
 * @returns Array of findings for each role conflict.
 */
function checkMethodologyExclusivity(plugins) {
    const findings = [];
    const coreRoles = new Map();
    for (const plugin of plugins) {
        if (!plugin.role || !plugin.role.startsWith("core:"))
            continue;
