...
description: |
  ...
  a community skill or create a project-specific one; managing the .claude/skills/ directory.
version: 1.0.0
...
- `--copy`: Copy files to `.claude/skills/` instead of symlinking (preferred for this project)
...
| Project skill | In `.claude/skills/` — committed with the codebase, shared by all agents |
...
| Custom skill | Created manually in `.claude/skills/<name>/SKILL.md` |
...
Every skill is a `SKILL.md` file in a directory under `.claude/skills/`:
...
- The skill would duplicate content already in `.claude/rules/` or docs
...
- Enforcement rules (those belong in `.claude/rules/`)
...
Verify: every directory in `.claude/skills/` has a SKILL.md, and every entry in the skills log matches an installed skill.
