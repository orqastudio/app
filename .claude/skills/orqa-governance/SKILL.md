...
description: |
  Orqa Studio governance patterns: artifact types, scanning pipeline, lesson promotion,
...
Orqa Studio's governance layer manages documentation, research, plans, lessons, rules, agents, skills, and hooks as browsable, scannable artifacts. Understanding this system is critical for anyone working on the governance features.
...
The enforcement engine loads rules from `.claude/rules/` and checks compliance:
...
impl EnforcementEngine {
    pub fn load(rules_dir: &Path) -> Result<Self, OrqaError> {
        // Scan .claude/rules/*.md, extract rule metadata from frontmatter
    }
...
Rules in `.claude/rules/` use simple frontmatter:
...
When `dogfood: true`, enhanced caution rules apply (see `.claude/rules/dogfood-mode.md`).
