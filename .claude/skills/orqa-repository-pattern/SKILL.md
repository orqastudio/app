...
description: |
  Orqa Studio's persistence layer using the repository pattern with rusqlite.
...
Orqa Studio persists structured data in SQLite via the `rusqlite` crate. Every database entity follows the same repository pattern: a module of free functions in `src-tauri/src/repo/` that accepts a `&Connection` and returns `Result<T, OrqaError>`.

## Repository Anatomy

Repositories are modules of free functions — not structs, not traits. Each module lives in `src-tauri/src/repo/` and is re-exported from `src-tauri/src/repo/mod.rs`.
...
```rust
// src-tauri/src/repo/project_repo.rs
...
Migrations live in `src-tauri/migrations/` as numbered SQL files. Schema creation uses `CREATE TABLE IF NOT EXISTS` for idempotency.
...
1. **Migration** — `src-tauri/migrations/NNN_description.sql` with `CREATE TABLE IF NOT EXISTS`
2. **Domain type** — `src-tauri/src/domain/<entity>.rs` with `Serialize, Deserialize, Debug, Clone`
3. **Repository** — `src-tauri/src/repo/<entity>_repo.rs` with standard functions
4. **Command** — `src-tauri/src/commands/<entity>_commands.rs` with thin wrappers
5. **TypeScript type** — `ui/lib/types/` matching the Rust struct
6. **Store** — `ui/lib/stores/<entity>.svelte.ts` calling `invoke()`
