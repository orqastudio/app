...
export class RuleEngine {
    ...
    loadRules() {
        ...
        for (const entry of fs.readdirSync(rulesDir)) {
            if (!entry.endsWith(".md") || !entry.startsWith("RULE-"))
                continue;
     ...
     */
    evaluate(event, content, filePath) {
        ...
        for (const rule of rules) {
            for (const entry of rule.enforcement) {
                ...
                if (entry.paths && filePath) {
                    const matches = entry.paths.some((glob) => this.matchGlob(filePath, glob));
                    if (!matches)
                        continue;
                }
                // Check pattern
                ...
                switch (entry.action) {
                    ...
                    case "warn":
                        result.warnings.push(entry.message ?? `Warning from ${rule.id}: ${rule.name}`);
                        break;
                    case "inject":
                        if (entry.skills) {
    ...
    parseEnforcement(yaml) {
        ...
        for (const block of entryBlocks) {
            ...
            for (const line of lines) {
                ...
                switch (key) {
                    case "event":
                        entry.event = value.trim();
                        break;
                    case "action":
                        entry.action = value.trim();
                        break;
                    case "pattern":
                ...
                if (key === "paths") {
                    ...
                    if (pathsMatch) {
                        entry.paths = pathsMatch[1].split(",").map((p) => p.trim().replace(/^["']|["']$/g, ""));
                    }
                }
                ...
                if (key === "skills") {
                    ...
                    if (skillsMatch) {
                        entry.skills = skillsMatch[1].split(",").map((s) => s.trim().replace(/^["']|["']$/g, ""));
                    }
                }
            }
