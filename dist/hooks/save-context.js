...
async function main() {
    ...
    const [epicsResult, tasksResult] = await Promise.allSettled([
        callDaemon("/query", { type: "epic", status: "active" }),
        callDaemon("/query", { type: "task", status: "in-progress" }),
    ]);
    ...
    if (activeEpics.length > 0) {
        ...
        for (const e of activeEpics)
            lines.push(`- **${e.id}**: ${e.title ?? e.id}`);
        lines.push("");
    }
    if (activeTasks.length > 0) {
        lines.push("## Active Tasks", "");
        for (const t of activeTasks)
            lines.push(`- **${t.id}** [${t.status ?? "active"}]: ${t.title ?? t.id}`);
        lines.push("");
    }
    if (existingState) {
        lines.push("## Previous Session State", "", existingState);
    }
    lines.push("", "## Recovery Instructions", "", "After compaction, re-read:", "1. The active epic files listed above", "2. The active task files listed above", "3. `.orqa/process/agents/orchestrator.md` for your role definition", "4. Any skills referenced by the current tasks");
