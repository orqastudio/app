...
async function main() {
    ...
    const [behavioralResult, agentResult] = await Promise.allSettled([
        callDaemon("/content/behavioral", {}),
        callDaemon("/content/agent", { match: agentType || "orchestrator" }),
    ]);
    const behavioralMessages = behavioralResult.status === "fulfilled" ? behavioralResult.value.messages : [];
    const preamble = agentResult.status === "fulfilled"
...
function classifyThinkingMode(userMessage, projectDir) {
    ...
    for (const line of result.stdout.split("\n").filter((l) => l.trim())) {
        ...
        let hits;
        try {
            hits = JSON.parse(String(textContent));
        }
        catch {
            continue;
        }
