...
async function main(): Promise<void> {
  ...
  try {
    schema = await callDaemon<SchemaResult>("/query", {
  ...
  logTelemetry("artifact-enforcement", "PreToolUse", startTime,
    missing.length === 0 ? "ok" : "warned",
    { file: relPath, missing_count: missing.length, missing: missing.map((r) => r.key) },
    projectDir,
  );
...
function extractRelationshipKeys(fm: Record<string, unknown>): Set<string> {
  const keys = new Set<string>();
  for (const val of Object.values(fm)) {
    if (!Array.isArray(val)) continue;
    for (const item of val) {
      if (item && typeof item === "object" && "type" in item) {
        keys.add(String((item as Record<string, unknown>)["type"]));
      }
    }
  }
  return keys;
