 ...
 */
function generateAgents(projectRoot: string, outputRoot: string, errors: string[]): string[] {
...
async function buildClaudemd(projectRoot: string, errors: string[]): Promise<string> {
	...
	const architectureCoreDoc = path.join(projectRoot, ".claude", "architecture", "core.md");
	let principlesBlock = "";
	if (fs.existsSync(architectureCoreDoc)) {
		try {
			const raw = fs.readFileSync(architectureCoreDoc, "utf-8");
			const match = raw.match(/##\s+2\.\s+Design Principles[\s\S]*?(?=\n---|\n##\s+[0-9])/);
			if (match) {
 ...
 */
function generateHooksJson(projectRoot: string, outputRoot: string, errors: string[]): string {
 ...
 */
function generateScripts(projectRoot: string, outputRoot: string, errors: string[]): string[] {
 ...
 */
function generateSkills(projectRoot: string, outputRoot: string, errors: string[]): string[] {
 ...
 */
function generatePluginJson(outputRoot: string, errors: string[]): string {
	...
	try {
		...
		fs.writeFileSync(outputPath, JSON.stringify(pluginManifest, null, 2) + "\n", "utf-8");
	} catch (err) {
		errors.push(`Failed to write plugin.json: ${err instanceof Error ? err.message : String(err)}`);
	}
...
function buildSettingsJson(): Record<string, unknown> {
	return {
		...
		permissions: {
			allow: ["Bash(./*)", "Read(./**)", "Write(./.state/team/**)"],
			deny: [
				"Read(./.env)",
				"Read(./.env.*)",
				"Read(./secrets/**)",
					...
					hooks: [
						{
							type: "command",
							command:
								"node \"$CLAUDE_PROJECT_DIR/scripts/validate-artifacts.mjs\" --hook --file \"$(echo $TOOL_INPUT | node -e \"process.stdin.on('data',d=>{const j=JSON.parse(d);console.log(j.file_path||j.content&&'skip'||'skip')})\"  2>/dev/null || echo skip)\"",
							timeout: 10,
					...
					hooks: [
						{
							type: "command",
							command:
								'bash -c \'FILE=$(echo "$TOOL_RESULT" | node -e "process.stdin.on(\\"data\\",d=>{try{const j=JSON.parse(d);console.log(j.filePath||\\"\\")}catch{console.log(\\"\\")}}" 2>/dev/null); if [[ "$FILE" == *.md ]] && [[ "$FILE" == */.orqa/* ]]; then npx markdownlint-cli2 "$FILE" 2>&1 || true; fi\'',
							timeout: 10,
 ...
 */
function generateSettingsJson(outputRoot: string, errors: string[]): string {
 ...
 */
function generateMcpJson(projectRoot: string, outputRoot: string, errors: string[]): string {
	const servers = aggregateServerConfigs(projectRoot, "mcpServers", errors);
	const outputPath = path.join(outputRoot, ".mcp.json");

	fs.writeFileSync(outputPath, JSON.stringify({ mcpServers: servers }, null, 2) + "\n", "utf-8");
 ...
 */
function generateLspJson(projectRoot: string, outputRoot: string, errors: string[]): string {
