...
description: "How multiple Svelte 5 rune stores coordinate in OrqaStudio without tight coupling. Covers store independence, component-level orchestration via $derived and $effect, circular dependency prevention, cross-store derived state, and initialization order. Use when: Wiring multiple stores together, adding cross-store coordination, debugging state synchronization issues, or planning new multi-store features."
user-invocable: true
---
