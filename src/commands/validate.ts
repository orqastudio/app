 ...
 */
function findRustBinary(projectRoot: string): string | null {
	const candidates = [
	...
	if (rustBinary) {
		...
		if (jsonOutput) {
			...
			try {
				...
				const errors = checks.filter((c: { severity: string }) => c.severity === "Error" || c.severity === "error").length;
