...
relationships:
  - type: cautions
    ...
    rationale: "Never split epics for progress optics — cautions delivery pipeline"
---

## Pattern

The orchestrator asked whether [EPIC-a60f5b6b](EPIC-a60f5b6b) should be split because of its size (28 tasks, 8 phases). The implicit assumption was that smaller epics are better. But epic size is not the relevant measure — goal completeness is. An epic that achieves half its goal is not "two epics done" — it's one incomplete epic.

## Fix

Update [RULE-b10fe6d1](RULE-b10fe6d1) or the `planning` skill to state explicitly:
