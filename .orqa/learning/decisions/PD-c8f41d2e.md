...
Per PD-a44384d1, the architecture has converged to:

| Component | Transport | Lifecycle | Port |
| ----------- | ----------- | ----------- | ------ |
| **Daemon** | HTTP | Long-running process (PID file in `.state/daemon.pid`) | `ORQA_PORT_BASE + 58` (default: 10258) |
...
Per PD-a44384d1, MCP is a CLI protocol mode (`orqa mcp`), not a separate service. Claude Code spawns it as an stdio subprocess via `.mcp.json`:
...
The current codebase still has a standalone `orqa-mcp-server` binary (Rust) that `orqa mcp` spawns. The target architecture (PD-a44384d1) is to make `orqa mcp` a native CLI mode that speaks MCP protocol directly. Either way, MCP remains stdio — no port, no health check needed from the connector.
...
1. **Daemon** — `orqa daemon start` (detached, PID file at `.state/daemon.pid`)
...
The dev controller uses a signal file (`.state/dev-signal`) for remote control:
...
3. Remove control file (`.state/dev-controller.json`)
4. Remove signal file (`.state/dev-signal`)
...
`.state/dev-controller.json` tracks running state:
...
When PD-a44384d1 is fully implemented (MCP/LSP as native CLI modes, search embedded in daemon):
...
1. **PD-a44384d1** — CLI as single interface, daemon as only service
2. **PD-1ef9f57c Q8** — Daemon is the business logic boundary, MCP/LSP are access protocols
