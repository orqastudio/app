...
- **CLI context**: Rules are loaded into agent context via `.claude/rules/` symlinks.
  ...
  `.claude/hooks/` was gitignored (RES-b484fa9f finding F-01).
...
Both [IDEA-3e497812](IDEA-3e497812) (structured enforcement engine) and [IDEA-05e5003d](IDEA-05e5003d) (native hooks / CLI
...
enforcement:
  - event: file
    ...
    action: block
    message: "No unwrap() in production code (RULE-9814ec3c)"
  - event: bash
    pattern: "git commit.*--no-verify"
    action: block
    message: "Never bypass pre-commit hooks (RULE-f609242f)"
```
...
- [x] [TASK-d48d6713](TASK-d48d6713): Add `enforcement` field to rule schema + add entries to key rules
- [x] [TASK-15cb18ee](TASK-15cb18ee): Create `orqa-plugin` repository with Claude Code plugin scaffold
- [x] [TASK-7565b47d](TASK-7565b47d): Implement rule engine core (loader, parser, pattern matcher)
- [x] [TASK-821ab799](TASK-821ab799): Implement agent & skill loading from `.orqa/process/`
- [x] [TASK-7c2638f0](TASK-7c2638f0): Implement PreToolUse hook (file + bash event enforcement)
- [x] [TASK-563e0955](TASK-563e0955): Implement SessionStart hook (orchestrator injection + session checks)
- [x] [TASK-243f8acc](TASK-243f8acc): Implement Stop hook (replaces pre-commit-reminder.sh)
- [x] [TASK-055c10f7](TASK-055c10f7): Implement `/orqa` command (basic version shipped)
- [x] [TASK-c541abcd](TASK-c541abcd): Document plugin installation and configuration

### Absorbed into EPIC-12fba656 (Enforcement Bootstrapping)

- [TASK-d38a48c9](TASK-d38a48c9): Integration testing → surpassed by [TASK-40fb1279](TASK-40fb1279)
- [TASK-1d45fac7](TASK-1d45fac7): Symlink removal → surpassed (premature, symlinks still needed alongside plugin)
- [TASK-577f3ed9](TASK-577f3ed9): Rust enforcement engine → surpassed by [TASK-8b51938b](TASK-8b51938b) (engine already exists)
- [TASK-a449b5c8](TASK-a449b5c8): Agent pipeline integration → surpassed by [TASK-8b51938b](TASK-8b51938b)
- [TASK-d4dade11](TASK-d4dade11): Governance UI violations → absorbed into [EPIC-12fba656](EPIC-12fba656)
