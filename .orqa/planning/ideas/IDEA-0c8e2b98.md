...
1. **Orphaned processes** — `cargo tauri dev` spawns Vite as a grandchild. When cargo dies, Vite survives, holding ports. This is a [known Tauri bug](https://github.com/tauri-apps/tauri/issues/10023) with no upstream fix. See [RES-44b34cba](RES-44b34cba).
...
`debugger/dev.mjs` currently provides reliable `stop` and `restart` using Node's `process.kill()` and port verification. This works today and can evolve into the full controller. See [RES-44b34cba](RES-44b34cba) for the research that informed this approach.
