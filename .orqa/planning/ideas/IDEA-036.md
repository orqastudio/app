...
EPIC-048 establishes an artifact graph covering `.orqa/` — governance artifacts and their relationships. But the real power comes from connecting governance to code: which tasks modified which files, which rules govern which modules, which decisions affected which components.
