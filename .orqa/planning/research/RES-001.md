...
|--------|-----------|--------------|
| Token extraction heuristics | [`onboarding.md`](RES-005) | Tier 1 scanning for tailwind.config.*, CSS custom properties, component library conventions |
| Token storage & format | [`design-tokens.md`](RES-003) | JSON in SQLite keyed by shadcn-svelte variable names, OKLCH format |
| Runtime theme application | [`design-tokens.md`](RES-003) | `document.documentElement.style.setProperty()` + mode-watcher |
| Extraction pipeline | [`design-tokens.md`](RES-003) | Three-source extraction: CSS :root > Tailwind config > generic vars. `csscolorparser` for conversion |
| Per-project persistence | [`design-tokens.md`](RES-003) | `project_themes` + `project_theme_overrides` tables in SQLite |
| shadcn-svelte theming mechanics | [`frontend.md`](RES-004) | CSS custom property convention, component library selection |

---

## Q1: OrqaStudio's Own Brand Identity

> **Authoritative brand direction:** [OrqaStudio Dual-Mode Identity Guidelines (v1)](DOC-047) supersedes the initial recommendations below. The guidelines define a dual-mode identity system (Product Mode: Industrial Minimal / Marketing Mode: Cyber-Technical), three logo marks with shared spark wedge geometry, and contextual spark colors (`--orqa-ember`, `--orqa-arc`, `--orqa-amber`, `--orqa-success`). The zinc base palette, Inter font stack, and system-preference dark mode decisions below remain valid.
...
**Implementation:** Already covered in [`design-tokens.md`](RES-003) Q2. `mode-watcher` with `ModeWatcher` component in root layout. Default mode: `'system'`. Fallback: `'dark'`.
...
**Why require user approval:** Auto-generating governance artifacts without user consent violates the progressive disclosure principle from [`onboarding.md`](RES-005). The user should feel in control of what goes into `.claude/`.
...
When Claude generates wireframes for a project (PlantUML Salt diagrams, as decided in [`wireframing.md`](RES-008)), the wireframes should represent the **project's** interface, not OrqaStudio's interface. This means:
...
- [Design Tokens Research](RES-003) -- Token format, storage, runtime application, extraction pipeline
- [Onboarding Research](RES-005) -- Codebase scanning heuristics, design system detection
- [Frontend Research](RES-004) -- shadcn-svelte theming, component library selection
- [Wireframing Research](RES-008) -- PlantUML Salt for wireframe generation
- [Claude Integration Research](RES-002) -- System prompt construction, Agent SDK sidecar
- [Architecture Decisions](DOC-001) -- [AD-013](AD-013) (frontend libraries), [AD-015](AD-015) (governance format)
- [MVP Specification](DOC-042) -- F-001 (project scan), F-009 (settings)
