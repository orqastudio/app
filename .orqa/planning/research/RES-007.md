...
**Decision:** Tauri v2 is fully capable. No blockers. Proceed with [AD-001](AD-001) (thick backend) and [AD-002](AD-002) (IPC boundary) as designed.
...
**Decision:** Use `invoke()` for CRUD, `Channel<T>` for streaming (Claude responses, progress), events for background notifications. Evaluate TauRPC for end-to-end type safety during Phase 1. → Extends [AD-002](AD-002).
...
**Decision:** Scoped permissions with `$HOME/**` base, sensitive path denials, pre-declared shell commands, keyring for API keys. → Needs new AD ([AD-007](AD-007)).
...
- [Architecture Decisions](DOC-001) — [AD-001](AD-001), [AD-002](AD-002) confirmed viable; [AD-007](AD-007) (security model) needed
- [Claude Integration Research](RES-002) — Streaming pipeline confirmed: reqwest → Channel → Svelte
- [Persistence Research](RES-006) — tauri-plugin-sql confirmed for SQLite
