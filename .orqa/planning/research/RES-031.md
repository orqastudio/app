...
status: surpassed
description: >
  Comprehensive audit aligning documentation, governance rules, and agent
  definitions with the three-layer architecture vision. Produced config-driven
  artifact scanning, simplified schema (plans merged into research), and clean
  traceability chain from Task to Epic to Milestone.
created: 2026-03-08
updated: 2026-03-08
milestone: MS-001
surpassed-by: "Merged into epic body per schema simplification"
tags: [vision, alignment, config, artifacts, three-layer-architecture, schema-simplification]
---
...
### Completed Tasks
- TASK-004: Product docs audit (8 files)
- TASK-005: Architecture/process docs audit (15 files)
- TASK-006: Rules and agents audit (35 files)
...
### Completed Tasks
- TASK-007: Rust types for ArtifactEntry/ArtifactTypeConfig
- TASK-008: Config-driven scanner in artifact_reader.rs (recursive file explorer pattern)
- TASK-009: Frontend config-driven navigation (deleted hardcoded constants)
- TASK-010: Task schema enhancement (assignee + skills fields)
...
- **Existing plans** move to `.orqa/planning/research/` and are marked `status: surpassed`
...
- **Remove `.orqa/planning/plans/` directory** after migration

### Tasks

- TASK-012: Update artifact-framework.md — remove Plan type, update schemas
- TASK-013: Migrate existing plans to research, update references
- TASK-014: Update artifact-lifecycle.md — remove plan status transitions, add `epic:` to task schema
- TASK-015: Update project.json artifacts config — remove plans entry
- TASK-016: Update all existing tasks to reference epics instead of plans

### Verification

- No `plan:` field in any task frontmatter (replaced by `epic:`)
- No files in `.orqa/planning/plans/` (moved to research, marked surpassed)
...
- TASK-011: Historical backfill (decision chains, surpassed research, lessons, surpassed tasks)
