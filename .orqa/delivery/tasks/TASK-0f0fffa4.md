...
`validate_file()` already calls `check_json_schema()` which delegates to `orqa_validation::checks::schema`. With plugin schemas ingested (TASK-061b5052), this path will produce real diagnostics. The remaining work is ensuring line-level positioning is accurate.
