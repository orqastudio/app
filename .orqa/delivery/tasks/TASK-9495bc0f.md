...
status: active
description: "Rename the delivery workflow skeleton to agile-methodology in the agile-workflow plugin"
relationships:
  ...
  - target: TASK-cd118e75
    type: depends-on
    rationale: "Plugin renames must complete before skeleton rename — agile-workflow dir must exist"
acceptance:
  - "plugins/agile-workflow/workflows/agile-methodology.workflow.yaml exists with name: agile-methodology"
  ...
  - "The workflow plugin: field is @orqastudio/plugin-agile-workflow"
...
The delivery workflow skeleton stays in `plugins/agile-workflow/` (the workflow-definition plugin) but must be renamed from `delivery` to `agile-methodology`. This is the skeleton that defines contribution point slots that stage-definition plugins fill.

## Knowledge Needed

- Read `plugins/agile-workflow/workflows/delivery.workflow.yaml` for the current skeleton structure
...
1. Rename `plugins/agile-workflow/workflows/delivery.workflow.yaml` to `plugins/agile-workflow/workflows/agile-methodology.workflow.yaml`
2. In the renamed file, change the `name:` field from `delivery` to `agile-methodology`
3. Update the `plugin:` field to `@orqastudio/plugin-agile-workflow`
...
- `test ! -f plugins/agile-workflow/workflows/delivery.workflow.yaml && echo PASS || echo FAIL`
- `grep 'name: agile-methodology' plugins/agile-workflow/workflows/agile-methodology.workflow.yaml | wc -l` should be 1
- `grep 'plugin: @orqastudio/plugin-agile-workflow' plugins/agile-workflow/workflows/agile-methodology.workflow.yaml | wc -l` should be 1
