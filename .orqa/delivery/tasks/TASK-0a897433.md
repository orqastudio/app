...
[RULE-b10fe6d1](RULE-b10fe6d1) defines the documentation gate as NON-NEGOTIABLE — all `docs-required` items must exist before an epic moves from `draft` to `ready`. Currently orchestrator-enforced via manual checking. Automate it.

## How

1. Create a validation script that reads an epic's `docs-required` frontmatter array
2. For each entry: if it matches an artifact ID pattern (e.g., `[AD-7121ec20](AD-7121ec20)`), check if that artifact exists in the graph. If it is a file path, check if the file exists on disk.
