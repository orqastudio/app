...
relationships:
  ...
  - target: TASK-cd118e75
    type: depends-on
    rationale: "Plugin renames must complete before moving schemas from agile-workflow and software-kanban"
...
acceptance:
  ...
  - "vision, persona, pillar, pivot, idea schemas and workflows no longer exist in agile-workflow"
...
- Read `plugins/agile-workflow/` (after TASK-1 rename) for:
...
- Read `plugins/agile-workflow/workflows/agile-methodology.workflow.yaml` (after TASK-2 rename) for the contribution point structure
...
3. Create `plugins/agile-discovery/schemas/` directory and move schemas:
   - Move vision, persona, pillar, pivot schemas from `plugins/agile-workflow/schemas/`
   - Move idea schema from `plugins/agile-workflow/schemas/`, rename type to `discovery-idea`
...
4. Create `plugins/agile-discovery/workflows/` directory and move workflows:
   - Move vision.workflow.yaml, persona.workflow.yaml, pillar.workflow.yaml, pivot.workflow.yaml from agile-workflow
   - Move idea.workflow.yaml from agile-workflow, rename type to `discovery-idea`
...
6. Remove moved schemas/workflows from agile-workflow and software-kanban manifests
...
- `test ! -f plugins/agile-workflow/workflows/idea.workflow.yaml && echo PASS || echo FAIL`
- `test ! -f plugins/agile-workflow/workflows/vision.workflow.yaml && echo PASS || echo FAIL`
