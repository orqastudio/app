...
relationships:
  - type: informed-by
    target: PILLAR-001
    rationale: Clarity Through Structure requires documentation to be visible and connected, not isolated
  - type: informed-by
    target: PILLAR-002
    rationale: Learning Through Reflection requires knowledge to accumulate in structured, traversable form
  - type: informed-by
    target: PILLAR-003
    rationale: Purpose Through Continuity requires agents to maintain purpose through grounding, which requires connected documentation
  - type: informed-by
    target: IMPL-055
    rationale: The missing graph integrity lesson revealed that documentation isolation is part of a broader graph connectivity problem
  - type: informs
    target: EPIC-064
