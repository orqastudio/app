...
- [ ] Separate dogfood context from base orchestrator — move dogfood behavioral rules (session lifecycle, sidecar self-edit warnings, restart protocol) out of the orchestrator prompt and into app-injected system prompt only. CLI sessions should NOT absorb dogfood-mode behavioral rules via `.claude/rules/` symlinks. The `[RULE-8ee65d73](RULE-8ee65d73)` (dogfood-mode) file should either be restructured or the app system prompt builder should be the sole injection point for dogfood behavioral context when `project.json` has `dogfood: true`.
