...
- **[EPIC-9a1eba3f](EPIC-9a1eba3f)**: Built the enforcement engine and companion plugin — rules are
  mechanically enforced via hooks, the plugin bridges `.orqa/` to Claude Code
- **[EPIC-56940fa8](EPIC-56940fa8)**: Shifted enforcement to structured thinking process gates, knowledge
  injection, and tooling ecosystem management
- **[EPIC-709a6f76](EPIC-709a6f76)**: Abstracted tool names into capabilities for provider-agnostic agents
...
The companion plugin (EPIC-9a1eba3f) becomes the primary consumer of the graph:
...
4. **Capability mapping** already includes `web_fetch` and `web_search` (RULE-8abcbfd5)
...
- [ ] [TASK-77bfa7e1](TASK-77bfa7e1): Add `docs` and `skills` fields to task schema
- [ ] [TASK-608d1faa](TASK-608d1faa): Backfill `docs` field on existing tasks from git history context
- [ ] [TASK-7a8690e2](TASK-7a8690e2): Backfill `skills` field on existing tasks from orchestrator injection table

### Orchestrator Simplification
- [ ] [TASK-bb4fa466](TASK-bb4fa466): Write graph-based orchestrator prompt (target: ~200 lines)
- [ ] [TASK-7a79c360](TASK-7a79c360): Extract dev server/worktree/hooks content to skills or docs
- [ ] [TASK-ccf61a62](TASK-ccf61a62): Extract project-specific requirements to graph-readable artifacts

### Plugin Graph Integration
- [ ] [TASK-80889691](TASK-80889691): Plugin reads task `docs` and `skills` fields for context injection
- [ ] [TASK-1ee2031e](TASK-1ee2031e): Plugin reads skill `scope` fields for path-based injection
- [ ] [TASK-9f9e3bea](TASK-9f9e3bea): Plugin extends graph on artifact creation (PostToolUse)

### Web Search Parity
- [ ] [TASK-a148b482](TASK-a148b482): Add `web_fetch` and `web_search` capabilities to research/planner agents
- [ ] [TASK-cae57b0a](TASK-cae57b0a): Add `sources` field to research schema for web references

### Core Protection
- [ ] [TASK-c39b12a7](TASK-c39b12a7): Pre-commit hook blocks core graph artifact modifications

### Decision Audit
- [ ] [TASK-bf368dca](TASK-bf368dca): Audit existing architecture decisions (AD-7121ec20 to AD-306eccf1) against [AD-f079c196](AD-f079c196)/039/040

### Project Setup Integration
- [ ] [TASK-4b76d8dd](TASK-4b76d8dd): Create skill for epic-requirement inference during project setup

### Verification
- [ ] [TASK-6429d9eb](TASK-6429d9eb): End-to-end test: task with docs/skills → plugin injects correct context
- [ ] [TASK-7a8598c6](TASK-7a8598c6): Verify orchestrator prompt works at ~200 lines (dogfood session)
