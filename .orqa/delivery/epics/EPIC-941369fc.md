...
Agent: orqa enforce override --rule RULE-xxx --reason "..."
  → { status: "requires-human-approval", request_id: "OVR-a1b2c3d4",
      approve_command: "orqa enforce approve OVR-a1b2c3d4" }
  → Request ID logged to enforcement log as "pending"

Human approves (separate action):
  → orqa enforce approve OVR-a1b2c3d4
  → Request ID updated in log to "approved"

Agent retries with approved ID:
  → orqa enforce override --rule RULE-xxx --request-id OVR-a1b2c3d4
  → CLI checks log: OVR-a1b2c3d4 is approved → override granted
...
The CLI stores approved request IDs in `tmp/enforcement-approvals.json`.
...
```json
// tmp/enforcement-approvals.json
{
  "OVR-a1b2c3d4": {
    "rule": "RULE-xxx",
    "reason": "Emergency hotfix",
...
2. `orqa enforce approve OVR-xxx` → writes to approvals file
3. `orqa enforce override --rule X --request-id OVR-xxx` → reads approvals,
...
Approve calls `orqa enforce approve OVR-xxx`. The agent's next
retry with `--request-id OVR-xxx` succeeds automatically.

**CLI (Claude Code) flow:**
The connector outputs: "Override requires human approval. Run this
in another terminal: `orqa enforce approve OVR-a1b2c3d4`"
