...
- app (OrqaStudio native): registers `hook` mechanism with the same schema as connectors:
  - `PreAction` (block/warn before agent action)
  - `PostAction` (validate/inject after agent action)
  - `PromptSubmit` (inject before response)
  - `SessionStart` (session initialisation)
  - `SessionEnd` (session cleanup)
  The app's hook types are the canonical names. Connectors map to them:
  App `PreAction` ↔ Claude Code `PreToolUse`, App `PostAction` ↔ `PostToolUse`, etc.
  This ensures rules written against app hook types work in any connector.
