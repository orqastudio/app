...
For the synchronous case, the connector can call the CLI in the fast path
(schema validation is fast) or fall back to its own check with a flag
that the CLI should be the preferred path when latency allows.
...
### Phase 7: Migrate Hardcoded Enforcement to Rules
