...
**Enforcement chain resolution:**
```
pre-commit (githooks)
  → consumes lint (coding-standards)
    → consumes clippy (rust), eslint (svelte), svelte-check (svelte)
  → consumes json-schema (core-framework)
  → consumes tool (cli) → orqa validate
```

The pre-commit hook reads all rules, finds enforcement entries matching
mechanisms it can run (directly or via consumed mechanisms), and executes
them in strength order. If a consumed mechanism's plugin is not installed,
