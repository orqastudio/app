...
**How it works:**
- Agent encounters enforcement that it believes should be overridden
- Agent requests override via `orqa enforce override --rule RULE-xxx --reason "..."`
- CLI prompts for human confirmation (interactive, not auto-approved)
- If approved: override logged with reason, user identity, timestamp
- If denied: enforcement stands, agent must comply
...
or any bypass without requesting human approval via `orqa enforce override`.
Any bypass without a logged override is an enforcement violation.
...
### Phase 12: Migrate Hardcoded Enforcement to Rules
