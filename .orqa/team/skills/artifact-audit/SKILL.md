...
For each agent in `.orqa/team/agents/`, read its `skills:` YAML list. Every entry must correspond to a directory in `.orqa/team/skills/` containing a `SKILL.md`. Skills listed on agent YAML should be Tier 1 portable skills only — project-specific `orqa-*` skills (except `orqa-composability`) should not appear on agent frontmatter; they are orchestrator-injected at task time per [RULE-026](RULE-026).
