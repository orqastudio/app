...
- **Sidecar self-edit warnings** — the sidecar they communicate through IS the sidecar they might be editing. Changes to `sidecar/src/` require extra care: verify the change won't break the active connection, and warn the user before modifying protocol types
...
| Backend/frontend dependency manifest | `src-tauri/Cargo.toml` / `package.json` |
| Backend entry point | `src-tauri/src/lib.rs` |
...
3. Add `PlanFrontmatter` struct to `src-tauri/src/domain/artifact.rs` (follows the generic `parse_frontmatter<T>` pattern)
