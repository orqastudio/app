...
summary: "Every agent must read its Required Reading documentation before any implementation. Required Reading is the first action after skill loading. If a document is missing or unreachable, the agent must stop and report — never silently skip. Replaces scattered 'Before Implementation' checklists with a consistent protocol across all agents."
tier: always
roles: [orchestrator, implementer, researcher, planner, reviewer, writer, designer]
priority: P1
tags: [required-reading, agent-protocol, documentation-loading]
relationships:
  - target: AD-c1e5a39e
    type: enforces
---
...
- The Governance Steward audits Required Reading lists for completeness during governance reviews
- If a Required Reading path is broken across multiple agents, the Governance Steward performs a bulk path update
...
- [RULE-ec9462d8](RULE-ec9462d8) (documentation-first) — documentation is the specification; code is the implementation
- [RULE-9814ec3c](RULE-9814ec3c) (coding-standards) — standards that agents must read
- [RULE-05ae2ce7](RULE-05ae2ce7) (architecture-decisions) — decisions that agents must comply with
