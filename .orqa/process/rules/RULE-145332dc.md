...
summary: "Lesson promotions (recurrence >= 2) and rule violations are ALWAYS CRITICAL priority — they preempt all delivery tasks. Stop current work, promote the lesson or correct the violation, trace to affected artifacts, then resume. Three enforcement layers: Stop hook, prompt injector, orqa audit CLI."
tier: always
roles: [orchestrator]
priority: P0
tags: [governance-priority, lesson-promotion, violations, critical]
relationships: []
---
...
2. **Prompt injection** — the connector's prompt injector (`connectors/claude-code/src/hooks/prompt-injector.ts`) injects behavioral rules into agent context so orchestrating agents cannot claim ignorance.
