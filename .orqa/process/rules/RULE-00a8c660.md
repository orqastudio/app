...
- Even a single-task team provides the structured findings-to-disk pattern ([RULE-d2e4f6a8](RULE-d2e4f6a8))
...
3. **Read findings files** from `tmp/team/<team-name>/` when agents complete
...
- **Session state** (`tmp/session-state.md`) — the orchestrator MUST write this directly. It has the full conversation context; delegating to an agent that must be re-told everything is wasteful and lossy.
