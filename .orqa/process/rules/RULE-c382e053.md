...
summary: "Fix root causes of type mismatches between layers at the layer that introduced the mismatch. Never paper over with aliases, shims, normalizer maps, or widened type unions. One canonical identifier per concept across all layers. Trace the value from origin to mismatch point using code search, fix at source, remove all downstream workarounds."
tier: stage-triggered
roles: [implementer, reviewer]
priority: P1
tags: [type-safety, root-cause, no-aliases, layer-consistency]
relationships:
  - target: AD-af88bb69
    type: enforces
---
...
- [RULE-0be7765e](RULE-0be7765e) (error-ownership) — a type mismatch is an error you own; fix it, don't work around it
- [RULE-af5771e3](RULE-af5771e3) (no-stubs) — alias entries are a form of fake data hiding real gaps
- [RULE-0d29fc91](RULE-0d29fc91) (code-search-usage) — use `search_regex` to find all usages of an identifier before renaming or removing it
