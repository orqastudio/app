...
Update `.state/session-state.md` whenever any of the following occur:
...
The stop hook's auto-generated state summary MUST NOT overwrite a richer orchestrator-maintained state. If `.state/session-state.md` was written by the orchestrator during the session, the stop hook should append a brief summary, not replace the file.
...
3. **Telemetry** — session state reminder events are logged to `.state/hook-metrics.json` for audit
...
- Writing `.state/session-state.md` only at session end
...
- [RULE-998da8ea](RULE-998da8ea) (dogfood-mode) — defines restart protocol which depends on session state
