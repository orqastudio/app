...
enforcement:
  ...
  - mechanism: tsc
summary: "Code quality enforcement belongs in linters, not regex hooks. Chain: documented standard -> linter config (clippy/eslint) -> hook trigger (pre-commit/make check) -> violation reported. Lint event entries are declarative traceability, not executors. Only use file enforcement for patterns no linter covers."
tier: stage-triggered
roles: [implementer, reviewer]
priority: P2
tags: [tooling, linting, enforcement-chain]
relationships:
  - target: "AD-2dc8ab96"
    type: "enforces"
---
...
```
Documented standard (RULE-9814ec3c, knowledge)
...
- [RULE-9814ec3c](RULE-9814ec3c) (coding-standards) — the standards this rule maps to linters
- [RULE-e1f1afc1](RULE-e1f1afc1) (knowledge-injection) — knowledge injection complements linter delegation
- [RULE-0be7765e](RULE-0be7765e) (error-ownership) — pre-commit hook enforcement
