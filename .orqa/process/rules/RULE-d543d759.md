...
summary: "Task complete only when ALL acceptance criteria met AND Reviewer verified. Implementers must run verification checklist. Reviewers: single unmet criterion is FAIL. Orchestrators: distinguish done/in-progress/blocked/partial. No self-certification. Partial work is acceptable — calling it done when it isn't is not."
tier: always
roles: [orchestrator, implementer, reviewer]
priority: P0
tags: [honest-status, verification, acceptance-criteria]
relationships: []
---
...
- Reporting a stub or placeholder as complete implementation (see RULE-af5771e3)

## Related Rules

- [RULE-af5771e3](RULE-af5771e3) (no-stubs) — stubs are the implementation-level form of dishonest reporting
- end-to-end-completeness — completion requires all layers working, not just one layer
- [RULE-8aadfd6c](RULE-8aadfd6c) (session-state-management) — session state must reflect actual completion status
