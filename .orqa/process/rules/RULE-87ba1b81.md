...
**If the orchestrator is writing anything other than coordination output, the system has failed to delegate.** See the delegation reference (DOC-7bdef310) for the full work-type-to-role mapping.
...
2. **Resolve capabilities** — Read the agent's `capabilities` field, determine the current context (CLI or App), and resolve to concrete tool names using [RULE-8abcbfd5](RULE-8abcbfd5) mapping tables. Include the resolved tool names in the delegation prompt.
...
This was promoted from [IMPL-2dc87f24](IMPL-2dc87f24) after 3 recurrences.
...
- [RULE-dd5b69e6](RULE-dd5b69e6) (knowledge-enforcement) — agents must load knowledge before starting work
- [RULE-5965256d](RULE-5965256d) (required-reading) — agents must read governing docs before implementation
- [RULE-5dd9decd](RULE-5dd9decd) (honest-reporting) — agents must report status accurately
- [RULE-b723ea53](RULE-b723ea53) (tool-access-restrictions) — constrains which tools each role may use
- [RULE-8abcbfd5](RULE-8abcbfd5) (provider-agnostic-capabilities) — capability → tool resolution at delegation time
