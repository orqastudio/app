...
enforcement:
  - mechanism: behavioral
    message: "Use worktree-based workflow with mandatory cleanup; never force-push to main; never use git reset --hard without explicit user request"
  - mechanism: hook
    type: PreToolUse
    event: bash
    action: block
  ...
  - mechanism: hook
    ...
    event: bash
    action: block
    pattern: "git\\s+push\\b[^|&;]*(main|master)\\b[^|&;]*(-f\\b|--force\\b)"
  - mechanism: hook
    type: PreToolUse
    event: bash
    action: block
...
summary: "Worktree-based git workflow: create worktree per task, work in isolation, merge after review, mandatory cleanup. Hooks block --no-verify, force-push to main, reset --hard, clean -f. Commit at natural boundaries (max 20 uncommitted files). Stash only for temporary rebase procedures. Data loss prevention checks before directory operations."
tier: always
roles: [orchestrator, implementer]
priority: P0
tags: [git, worktree, safety, data-loss-prevention]
relationships:
  - target: AD-4ea9a290
    type: enforces
---
...
# 3. Request Reviewer approval BEFORE merging
...
- [RULE-d2c2063a](RULE-d2c2063a) (development-commands) — `make dev` and `make restart-tauri` commands that interact with the worktree lifecycle
- [RULE-0be7765e](RULE-0be7765e) (error-ownership) — pre-commit hook enforcement; `--no-verify` is forbidden under both rules
- [RULE-9814ec3c](RULE-9814ec3c) (coding-standards) — defines the quality checks the pre-commit hook enforces; commits must be clean
- [RULE-998da8ea](RULE-998da8ea) (dogfood-mode) — restart protocol and session-ending behavior that intersects with commit discipline
