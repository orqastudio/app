...
summary: "Orchestrator must protect its context window: delegate rather than accumulate, use targeted reads with offset/limit, summarize agent results instead of echoing, complete one task before starting next, persist intermediate results to .state/session-state.md. Never read entire large files or hold implementation details across turns."
tier: always
roles: [orchestrator]
priority: P1
tags: [context-management, efficiency, delegation]
relationships:
  - target: AD-efe10e1d
    type: enforces
  - target: AD-9baf6ee9
...
5. **Use session state.** Write intermediate results to `.state/session-state.md` rather than holding them in context across turns.
...
- [RULE-87ba1b81](RULE-87ba1b81) (agent-delegation) — orchestrator delegates, doesn't implement
- [RULE-0d29fc91](RULE-0d29fc91) (code-search-usage) — use search tools instead of reading files
