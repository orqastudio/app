...
| Boundary | Action |
|----------|--------|
| Task completion | Commit all task deliverables + push |
| Epic completion | Commit + update epic/task statuses + push |
| Session end | Commit ALL uncommitted changes + push before ending |
| Governance-only work (rules, docs, artifacts) | Commit at least every 20 files or at each logical milestone + push |

### Push After Commit (NON-NEGOTIABLE)

Every commit MUST be followed by `git push` to the remote. This ensures work is never lost to local-only state. The push is part of the commit action, not a separate step.

- After every `git commit`, run `git push` immediately
- If the branch has no upstream, use `git push -u origin <branch>` on the first push
- A commit that exists only locally is not safe — treat it as uncommitted work until pushed
- If the push fails (network issues, rejected push), resolve the issue before continuing other work
...
- Committing without pushing — every commit must be pushed to the remote immediately
