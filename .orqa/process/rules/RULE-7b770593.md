...
```
exploring ──> active ──> completed
                     └──> surpassed
```

- `exploring → active`: Decision reviewed and approved by the user
- `active → completed`: Decision is fully implemented and no longer evolving — the work it drove is done
- `active → surpassed`: A new decision replaces this one — both the new and old artifacts MUST be updated in the same commit

**Status definitions:**

| Status | Meaning |
|--------|---------|
| `exploring` | Under investigation — the decision is being researched and debated |
| `active` | Accepted and in effect — agents must comply |
| `completed` | Fully implemented, no longer evolving — historical record of a finished choice |
| `surpassed` | Replaced by a newer decision — the `evolves-into` field points to the successor |
...
**Supersession rule:** When a new decision replaces an active decision, both the new artifact (`evolves-from: AD-<old>`) and the old artifact (`status: surpassed`, `evolves-into: AD-<new>`) MUST be updated in the same commit. A one-sided supersession is an integrity violation.
