...
relationships:
  - target: PILLAR-001
    type: grounded
    rationale: Task-first audit trail creates structured work traceability
  - target: RULE-004
    type: enforced-by
    rationale: RULE-004 referenced in decision body
  - target: RULE-027
    type: enforced-by
    rationale: RULE-027 referenced in decision body
  - target: DOC-036
    type: documented-by
...
- [RULE-004](RULE-004) (artifact lifecycle) needs updating to respect the project config
...
- Update [RULE-004](RULE-004): Epic enforcement reads project config instead of hardcoding
- Update [RULE-027](RULE-027): "Structure before work" minimum is "task" when `epics-required: false`
