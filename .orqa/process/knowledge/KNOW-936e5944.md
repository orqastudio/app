...
status: "archived"
archived_reason: "Describes pre-migration .orqa/ structure (principles/, discovery/, delivery/, process/ layout, app/.orqa/ as core layer, agents/ directory). Target architecture uses stage-first organization per .claude/architecture/governance.md. Agents are now ephemeral, grounding/ replaced by tier:always knowledge in plugins. The three-artifact-home model is replaced by plugin-composed content installation."
