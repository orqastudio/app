...
- **How it works:** Agents write findings to disk (`.state/team/*/task-*.md`). Orchestrator reads only a structured summary (status, key decisions, follow-ups) — not the full findings.
...
- **Session state:** Written to disk (`.state/session-state.md`), referenced when needed
...
- Rule "RULE-b10fe6d1" loaded 8 times, referenced 2 times (75% waste)
