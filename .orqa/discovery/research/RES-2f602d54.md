...
- `CLAUDE.md` (symlink to `plugins/core/agents/AGENT-1dab5ebe.md`): **370 lines**
...
- **Problem**: The "critical rules" set (RULE-00a8c660, RULE-532100d9, etc.) is injected on EVERY prompt regardless of context
...
```
.claude/rules/RULE-00a8c660.md → .orqa/process/rules/RULE-00a8c660.md
.claude/rules/RULE-532100d9.md → .orqa/process/rules/RULE-532100d9.md
... (56 more)
```
...
3. **No rule hierarchy**: RULE-532100d9 (agent delegation) is loaded the same way as RULE-22783309 (artifact ID semantics). Both are critical, but they're used differently:
...
2. **Critical rule selection**: Always injects RULE-00a8c660, RULE-532100d9, RULE-5e03e67b, RULE-878e5422, RULE-9daf29c0
...
```typescript
// Agent completes task, writes findings to tmp/team/<team>/task-<id>.md
...
**What**: Enforce findings-to-disk pattern — orchestrator NEVER reads agent findings into its own context. All findings stay in `tmp/team/*/` files.
...
**Implementation**:
- Add enforcement entry to prevent Read of `tmp/team/*/task-*.md` files by orchestrator
...
- Largest rules: RULE-7b770593 (artifact-lifecycle) at 650 lines
