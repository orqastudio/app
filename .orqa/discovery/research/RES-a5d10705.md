...
|--------|-----------|--------------|
| Token extraction heuristics | [`onboarding.md`](RES-4ae0750b) | Tier 1 scanning for tailwind.config.*, CSS custom properties, component library conventions |
| Token storage & format | [`design-tokens.md`](RES-2d91e2c2) | JSON in SQLite keyed by shadcn-svelte variable names, OKLCH format |
| Runtime theme application | [`design-tokens.md`](RES-2d91e2c2) | `document.documentElement.style.setProperty()` + mode-watcher |
| Extraction pipeline | [`design-tokens.md`](RES-2d91e2c2) | Three-source extraction: CSS :root > Tailwind config > generic vars. `csscolorparser` for conversion |
| Per-project persistence | [`design-tokens.md`](RES-2d91e2c2) | `project_themes` + `project_theme_overrides` tables in SQLite |
| shadcn-svelte theming mechanics | [`frontend.md`](RES-80a476c7) | CSS custom property convention, component library selection |
...
**Implementation:** Already covered in [`design-tokens.md`](RES-2d91e2c2) Q2. `mode-watcher` with `ModeWatcher` component in root layout. Default mode: `'system'`. Fallback: `'dark'`.
...
**Why require user approval:** Auto-generating governance artifacts without user consent violates the progressive disclosure principle from [`onboarding.md`](RES-4ae0750b). The user should feel in control of what goes into `.claude/`.
...
When Claude generates wireframes for a project (PlantUML Salt diagrams, as decided in [`wireframing.md`](RES-d067c8c2)), the wireframes should represent the **project's** interface, not OrqaStudio's interface. This means:
...
- [Design Tokens Research](RES-2d91e2c2) -- Token format, storage, runtime application, extraction pipeline
- [Onboarding Research](RES-4ae0750b) -- Codebase scanning heuristics, design system detection
- [Frontend Research](RES-80a476c7) -- shadcn-svelte theming, component library selection
- [Wireframing Research](RES-d067c8c2) -- PlantUML Salt for wireframe generation
- [Claude Integration Research](RES-b666c725) -- System prompt construction, Agent SDK sidecar
- Architecture Decisions -- [AD-33e315cc](AD-33e315cc) (frontend libraries), [AD-0dfa4d52](AD-0dfa4d52) (governance format)
