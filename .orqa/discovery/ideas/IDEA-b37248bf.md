...
Code and governance currently live in separate systems — `.orqa/` contains the artifact graph while `backend/`, `ui/`, and `sidecar/` contain the implementation. There is no machine-readable link between a Rust function and the task it implements, or between a Svelte component and the epic it delivers. A comment-based reference structure (e.g., `// @orqa TASK-a4b53eaa`) would inject the codebase into the artifact graph, enabling traceability from governance artifact down to implementation line. Unified search across both code and governance would become possible, potentially replacing the separate ChunkHound/native search systems with a single graph-aware search layer.
