...
description: "Move session state from .state/session-state.md into the .orqa/ directory so it becomes a first-class artifact visible in the app's artifact navigation. Enables users to browse session history, see pending decisions, and understand what was happening across sessions — all within the OrqaStudio UI."
status: captured
created: 2026-03-13
updated: 2026-03-13
horizon: next
research-needed:
  ...
  - "How does this interact with the existing .state/session-state.md convention? Migration path?"
...
Session state currently lives in `.state/session-state.md` — a gitignored, invisible file that only agents read. This means:
...
**Directory:** `.orqa/implementation/sessions/` (or `.orqa/sessions/`)
...
This gives Purpose Through Continuity (PILLAR-a6a4bbbb) a permanent, browsable record — not just a transient file.
