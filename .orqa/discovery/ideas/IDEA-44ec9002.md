...
A new artifact type `PERSONA-NNN` in `.orqa/implementation/personas/` with a schema covering name, role, goals, frustrations, and pillar alignment. Epics and ideas gain an optional `personas` field listing which personas the work serves. The artifact browser gains a Personas section. The Designer role gains persona-awareness when generating UI plans.
