...
Module tree, domain types, command handlers, and dependency graph for `app/src-tauri/src/`. Rust owns the domain model [AD-7121ec20](AD-7121ec20). All functions return `Result\<T, E\>` [AD-2d58941b](AD-2d58941b). No `unwrap()`, `expect()`, or `panic!()` in production code.
