...
relationships:
  - target: RULE-f609242f
    type: documents
    rationale: This document is the source-of-truth for the git workflow and worktree lifecycle that RULE-f609242f enforces
  - target: RULE-b10fe6d1
    type: documents
    rationale: This document covers the task process and status transitions governed by RULE-b10fe6d1
---
...
At session end, the orchestrator writes a handoff note to `.state/session-state.md` (gitignored).
