...
All search commands return `Result\<T, OrqaError\>`. Tauri serialises `OrqaError` to the frontend via the `Serialize` derive on `OrqaError`. This follows the project-wide error propagation pattern (see [AD-2d58941b](AD-2d58941b)) — no mapping to `String` at the command boundary.
