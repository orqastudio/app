...
relationships:
  - target: EPIC-d45b4dfd
    type: documents
    rationale: Documentation page references EPIC-d45b4dfd
---
...
The types are defined in `ui/src/lib/types/artifact-graph.ts` and mirror the Rust structs in `app/src-tauri/src/domain/artifact_graph.rs`. Field names use `snake_case` to match the Rust serde serialization output.

```typescript
interface ArtifactNode {
    /** Frontmatter `id` field (e.g. "EPIC-d45b4dfd"). */
    id: string;
    /** Relative path from the project root (e.g. ".orqa/implementation/epics/EPIC-d45b4dfd.md"). */
    path: string;
...
Resolve an artifact node by its frontmatter ID (e.g. `"[EPIC-d45b4dfd](EPIC-d45b4dfd)"`). Returns `undefined` when no artifact with the given ID exists in the graph.

```typescript
resolveByPath(path: string): ArtifactNode | undefined
```

Resolve an artifact node by its relative file path (e.g. `".orqa/implementation/epics/[EPIC-d45b4dfd](EPIC-d45b4dfd).md"`). Returns `undefined` when no artifact at that path has been indexed.
...
Read the raw markdown content of an artifact file from disk. This is always an async IPC call — the SDK does not cache file content. Pass the relative path from the project root (e.g. `".orqa/implementation/epics/[EPIC-d45b4dfd](EPIC-d45b4dfd).md"`).
...
const node = artifactGraphSDK.resolve("EPIC-d45b4dfd");
if (!node) {
    console.warn("EPIC-d45b4dfd not found in artifact graph");
    return;
}
...
const ARTIFACT_PREFIX_MAP: Record<string, string> = {
    "EPIC": ".orqa/implementation/epics",
    "TASK": ".orqa/implementation/tasks",
    // ...
};
```
