...
description: |
  How to read and use OrqaStudio governance data — artifact graph, relationship
  vocabulary, status model, and enforcement rules. Use when: navigating the artifact
  graph, creating or modifying governance artifacts, querying knowledge via MCP.
tier: always
summary: "How to read and use OrqaStudio governance data — artifact graph, relationship vocabulary, status model, and enforcement rules."
status: active
created: 2026-03-28
updated: 2026-03-28
...
graph_read({ path: ".orqa/process/knowledge/KNOW-NNN.md" })

# Get a knowledge artifact's relationships (which agents use it)
graph_relationships({ id: "KNOW-f0c40eaf" })  # planned — requires SurrealDB graph tools
```
