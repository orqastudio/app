...
description: "How the .orqa/ governance structure works: artifact layering, config-driven\ndiscovery, relationship vocabulary, status lifecycle, lesson pipeline, and\ndirectory conventions. Use when: creating or maintaining governance artifacts,\nunderstanding how the three artifact layers merge, or working with the\nrelationship graph.\n"
status: archived
archived_reason: "Describes pre-migration .orqa/ structure (principles/, discovery/, delivery/, process/ layout, app/.orqa/ as core layer, agents/ directory). Target architecture uses stage-first organization per DOC-fd3edf48 (governance architecture). Agents are now ephemeral, grounding/ replaced by tier:always knowledge in plugins. The three-artifact-home model is replaced by plugin-composed content installation."
...
priority: "P1"
summary: |
  Governance patterns: how .orqa/ structure works — artifact layering,
  config-driven discovery, relationship vocabulary, status lifecycle, lesson
  pipeline, directory conventions.
---
...
5. **Artifact IDs** (all-caps like `EPIC-797972a7`, `PD-0dfa4d52`) are preserved as-is, not humanised
