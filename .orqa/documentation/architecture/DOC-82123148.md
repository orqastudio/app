...
Each target is a hand-written artifact that represents what the finished generation pipeline would produce. These are built FIRST and everything else validates against them. Migration validation is currently in progress (Phase 10) — some targets have been matched by generation pipelines while others are still being developed toward.
