...
### P7: The Resolved Workflow Is a Record

After plugin contributions are merged, the resolved workflow is a deterministic SurrealDB record, exportable as YAML via `orqa export`. The runtime reads from SurrealDB; it does not re-merge at runtime.
...
**Rust** is the base language for all libraries, the CLI, and the daemon. **TypeScript** is the frontend language only (SvelteKit UI, `libs/types`, `libs/sdk`, svelte-components, and plugin custom views). This is a clean boundary: Rust below, TypeScript only at the UI surface.

Connectors are a special case: the connector **source** is Rust (calls engine crates directly), but the connector **generates** output in whatever language the target tool requires (e.g., TypeScript/JS for Claude Code). The language of generator output is determined by the target tool's plugin format — it does not indicate that the connector itself is TypeScript.
...
**App + Sidecar:** The desktop application (SvelteKit frontend + Tauri backend) hooks into the engine libraries for all business logic. It is the custom UI for interacting with the engine. Because the app has no built-in LLM inference, it requires **sidecar plugins** that integrate with LLM providers. The tool executor in the app provides a consistent set of tools via MCP to any sidecar instance. Connectors can map these to the third-party tool's native tools for better performance. Enforcement plugin pages in the app dynamically render Run and Fix controls from the installed plugin registry — the Rust backend dispatches to `orqa enforce --<engine>` or `orqa enforce --<engine> --fix`. No enforcement tool is hardcoded in the frontend (P1).

**CLI:** A thin Rust binary wrapping the engine crates. An access method, not business logic. Commands like `orqa install`, `orqa enforce`, `orqa graph` delegate entirely to the engine crates. `orqa enforce` is the universal enforcement entry point — it reads the plugin registry at runtime to build its flag set and dispatch to installed engines. Flags like `--eslint`, `--clippy`, `--vale` are dynamically generated from installed plugins; none are hardcoded in the CLI source. `orqa enforce --staged` is what the pre-commit git hook calls.

**Connector:** A Rust binary that calls engine crates directly to **generate** a tool-native plugin (e.g., a Claude Code Plugin in TypeScript/JS). Once generated, the third-party tool interacts with the engine directly via CLI/MCP. The connector also **watches** for changes to plugins, rules, and composition and regenerates in real time. The generated output goes where the third-party tool expects it (e.g., `.claude/` for Claude Code). Connector file watcher registrations are managed by the daemon — the daemon reads watcher declarations from the connector's manifest and invokes the connector's generator when watched files change.
...
**Two storage layers serve distinct purposes:**

- **SurrealDB** — source of truth for the artifact graph. All governance artifacts (epics, tasks, rules, lessons, decisions, knowledge, docs) are stored as typed nodes with graph edges in SurrealDB. This enables native graph traversals, ancestry tracing, cycle detection, health metrics, and the artifact browser. Markdown files in `.orqa/` are the human-editable surface; SurrealDB is the authoritative runtime representation.
- **SQLite (via SeaORM)** — operational data that is not part of the artifact graph: projects, sessions, messages, settings, themes, violations, health snapshots, devtools sessions, events, and issue groups. SeaORM provides type-safe migrations and entity models for this layer.
