...
Rules in `.orqa/learning/rules/RULE-NNN.md` can have an `enforcement` array in their YAML frontmatter. Each entry defines a pattern that is evaluated by the plugin's PreToolUse hook before tool calls execute.
...
1. Open the rule file in `.orqa/learning/rules/`
