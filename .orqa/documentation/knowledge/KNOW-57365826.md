...
id: KNOW-57365826
type: knowledge
title: schema-lookup-before-write
...
relationships:
  - target: AGENT-4c94fe14
    type: employed-by
    rationale: "Auto-generated inverse of employed-by relationship from AGENT-4c94fe14"
tier: "always"
roles:
  - "*"
tags:
  ...
  - "frontmatter"
priority: "P1"
summary: "Schema-first frontmatter rule: agents must query plugin schemas via MCP or read orqa-plugin.json before creating or modifying any artifact. Status vocabularies are defined by plugins, not hardcoded — memorizing values causes drift. Query via graph_query for the artifact type, or read the plugin's statusTransitions section. Applies to all status updates, new artifact creation, and any enum-constrained frontmatter field."
---
...
Read `schema.json` in the artifact's directory (e.g., `.orqa/implementation/tasks/schema.json`). The `status` field's `enum` array lists valid values.
