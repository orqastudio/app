...
- `graph_query` — find the current milestone, epics, and task state (planned — requires SurrealDB graph tools)
- `graph_resolve` — load full artifact details including dependencies (planned — requires SurrealDB graph tools)
- `graph_relationships` — map what depends on what before sequencing (planned — requires SurrealDB graph tools)
- Pillar gate questions — every proposed task must answer at least one gate question

Plans follow the documentation-first principle (RULE-ec9462d8 (documentation-first implementation)): the plan document is written before any implementation task is created, and implementation tasks reference the plan.
...
- Planning output (epics, tasks) goes into `.orqa/implementation/` with full frontmatter

---

## Governance

- RULE-ec9462d8 (documentation-first implementation): plans are written before task artifacts
- RULE-1b238fc8 (vision alignment): every planned task must serve at least one pillar
