...
Artifact types are defined in `core.json`. They fall into three management layers per PD-b2f1f829. **Platform** types ship with the app (immutable). **Project** types are created by the user and AI. **Plugin** types are installed from the ecosystem.
...
| ------- | ----------- | --------------------- | ---------------- |
| **Discovery** | `.orqa/discovery/` | Why does this project exist? Who is it for? What could we build? What do we know? | Vision, Pillar, Persona, Idea, Research, Wireframe |
| **Planning** | `.orqa/planning/` | How do we approach this? What are the options? | Plans, Wireframes (planning stage) |
| **Implementation** | `.orqa/implementation/` | What are we building? What's the plan? What's done? | Milestone, Epic, Task |
| **Learning** | `.orqa/learning/` | How do we work? What constraints apply? What have we learned? | Decision, Rule, Lesson |
| **Documentation** | `.orqa/documentation/` | What does the user need to know? What does the developer need to know? | Document (DOC) |

### Discovery Layer

The discovery layer (`.orqa/discovery/`) is the **foundational "why and what" layer**. It contains artifacts that define the project's identity, values, target audience, and candidate work.
...
| `ideas/` | Idea | Candidate future work that needs investigation before committing. |
| `research/` | Research | Investigations, spikes, and explorations that inform decisions. |
| `wireframes/` | Document (DOC) | UI wireframes and sketches. |

Discovery artifacts like pillars and vision change through deliberate **pivots** (see the `pivot` type) when the project's direction evolves. The `revises` / `revised-by` relationship connects pivots to the foundational artifacts they modify.
...
│   └── wireframes/                 # DOC-NNN — UI wireframes and sketches
├── planning/
│   └── wireframes/                 # DOC-NNN — planning-stage wireframes
├── implementation/
│   ├── milestones/                 # MS-NNN — strategic goals
│   ├── epics/                      # EPIC-NNN — trackable work units
│   └── tasks/                      # TASK-NNN — individual implementation units
├── learning/
│   ├── decisions/                  # PD-NNN — planning and principle decisions
│   ├── rules/                      # RULE-NNN — enforceable constraints
│   └── lessons/                    # IMPL-NNN — implementation learnings
└── documentation/
    ├── architecture/               # DOC-NNN — architecture docs
    ├── platform/                   # DOC-NNN — platform docs
    ├── project/                    # DOC-NNN — project-specific docs
    ├── guides/                     # DOC-NNN — how-to guides
    └── reference/                  # DOC-NNN — reference material
```
...
All artifact types share the same 12-status vocabulary, defined in `project.json` (see PD-b8c11db0). Types use subsets of these statuses appropriate to their lifecycle.
...
| Decision | `AD-NNN` | PD-7121ec20, PD-ee2910b1 |
...
| PD-b2f1f829 | Three-layer artifact model (app-fixed, app-required, project-scoped) |
| PD-6ccfb736 | Canonical relationship vocabulary (relationships are the only connections) |
| PD-b8c11db0 | Canonical status model (12 universal statuses) |
| PD-a1c2ca06 | Four enforcement layers (app, scanners, integrity, git hooks) |
