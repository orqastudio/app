...
| **Implementation Team** | Specialized agents | Implement delegated tasks end-to-end. Load knowledge. Run quality checks. Report results. |
...
All 7 universal roles are defined in `.orqa/process/agents/`. Each role is invoked via the Task tool with the appropriate `subagent_type`, and domain capability is loaded via knowledge at delegation time (see [AD-774cc3d0](AD-774cc3d0)).
...
| **Reviewer** | Checks quality, compliance, correctness | `orqa-code-search`, `composability` + review knowledge loaded by orchestrator | Code quality, QA, UX compliance, security audits |
...
Orchestrator-injected Tier 2 knowledge narrows the Implementer and Reviewer roles to specific domains. See the delegation guide above and [AD-774cc3d0](AD-774cc3d0) for the full knowledge-loading model.

---

## Knowledge Directory

Knowledge artifacts are domain-specific instruction sets stored in `.orqa/process/knowledge/`. They follow the open [Agent Skills](https://agentskills.io) standard and must be portable — no OrqaStudio-specific architectural rules in knowledge artifacts.
...
For full provenance and date-added information, see Knowledge Log.
...
| `.orqa/process/knowledge/` | Technology patterns — portable, no OrqaStudio-specific rules |
