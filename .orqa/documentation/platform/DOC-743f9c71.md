...
id: DOC-743f9c71
type: doc
title: Tech Debt Management Guide
domain: guides
category: how-to
status: active
updated: 2026-04-13
...
Tech debt is the gap between what the code does and what the schemas define. OrqaStudio is schema-driven — `libs/types/src/platform/core.json` and plugin manifests (`orqa-plugin.json` per plugin) are the source of truth for artifact types, relationships, and validation rules. Any code that hardcodes these values instead of reading them from the schema is debt.
...
When you add a new relationship to `libs/types/src/platform/core.json`, every file that references relationship keys must be checked. The schema changed but the code didn't follow.
