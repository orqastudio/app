...
A dropdown control in the Chat Panel header for switching between sessions. Shows the active session title (clickable to open), a list of recent sessions with search filter, and a "New Session" button. Replaces the dedicated Sessions Panel from the previous four-zone layout [AD-85d45674](AD-85d45674). `Ctrl+N` creates a new session directly.
