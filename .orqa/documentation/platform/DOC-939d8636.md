...
A specialized AI persona with a defined role, skill set, and domain boundary. Agents are defined as markdown files in `app/.orqa/process/agents/` (core agents) with YAML frontmatter specifying `employs` relationships to skills and `observes` relationships to monitored artifacts. Agents are provider-agnostic definitions.

**Examples:** orchestrator, planner, implementer, reviewer, designer, researcher, writer, governance-steward.

### Rule

A constraint that agents must follow during implementation. Rules are markdown files in `.orqa/learning/rules/` (project) or `app/.orqa/process/rules/` (core) with YAML frontmatter including enforcement entries and an `enforces` relationship to the governing decision. Rules are the primary mechanism for enforcing governance.

**Examples:** "All Rust code must use Result types, not unwrap," "Every task must pass through a review gate before completion."

### Skill

A reusable knowledge package that provides domain-specific context to agents. Skills are directories containing a `KNOW.md` file, living in `app/.orqa/process/knowledge/` (core), `plugins/*/knowledge/` (plugin), or `.orqa/learning/` (project). Each skill may have a paired documentation artifact connected via `synchronised-with`. Skills are loaded on demand based on agent `employs` relationships and file-pattern injection rules.
...
**Note:** Decisions, lessons, and rules live in `.orqa/learning/decisions/`, `.orqa/learning/lessons/`, and `.orqa/learning/rules/` respectively. Discovery artifacts (ideas, research, personas, pillars, vision) live in `.orqa/discovery/`. Implementation artifacts (milestones, epics, tasks) live in `.orqa/implementation/`.
...
A documented implementation-level learning captured when something goes wrong or a non-obvious pattern is discovered. Individual lesson files stored in `.orqa/learning/lessons/` as `IMPL-NNN.md` with a recurrence count and promotion status. Format: `IMPL-NNN`.
...
The persistence strategy for OrqaStudio has two distinct concerns:

- **Governance artifacts** — stored in SurrealDB as the source of truth (queryable graph of typed artifacts and relationships). Git provides version history and audit trails as automatic background infrastructure. The markdown files in `.orqa/` are exportable representations via `orqa export`, not the primary store.
- **Operational data** — sessions, messages, settings, themes, violations, and health snapshots are stored in SQLite via SeaORM, providing fast local access for UI queries.

SurrealDB handles the artifact graph. SQLite handles application operational data. These are separate stores with separate purposes.
...
A dropdown control in the Chat Panel header for switching between sessions. Shows the active session title (clickable to open), a list of recent sessions with search filter, and a "New Session" button. Replaces the dedicated Sessions Panel from the previous four-zone layout [PD-85d45674](PD-85d45674). `Ctrl+N` creates a new session directly.
...
The single source of truth for platform artifact types, relationship definitions, and semantic categories. Located at `libs/types/src/platform/core.json` (distributed as `libs/types/dist/platform/core.json`). Loaded at runtime by both the Rust backend (embedded via `include_str!()`) and TypeScript (imported as JSON). No artifact types or relationship keys are hardcoded in any code path.
...
- [VISION-4893db55](VISION-4893db55) — Product vision and mission
- Artifact Framework (DOC-28344cd7) — Comprehensive artifact type and relationship reference
- Enforcement Architecture (DOC-b11d4f61) — Five-layer enforcement system
- Status & Workflow (DOC-61ecc85e) — Unified 12-status vocabulary
