...
relationships:
  ...
  - target: RULE-3c2da849
    type: documents
    rationale: Documentation page references RULE-3c2da849
...
hooks/
  ...
  scripts/
    ...
    prompt-injector.ts  # UserPromptSubmit: artifact graph traversal
...
- **Knowledge injection:** `.state/.injected-skills.json` — prevents re-injecting the same knowledge content on repeated matches
- **Artifact ID injection:** `.state/.injected-ids.json` — prevents re-emitting the same artifact IDs when referenced again

Both files are ephemeral (cleared when `.state/` is cleaned between sessions).
...
| Graph traversal | Simple edge crawl in `prompt-injector.ts` | Full bidirectional graph in `artifact_graph.rs` |
...
Core graph artifacts (schemas, orchestrator prompt, core knowledge) are protected by [RULE-3c2da849](RULE-3c2da849). Only the dogfood exception allows modification.
...
**Tracking:** [IDEA-9b26ba54](IDEA-9b26ba54)
...
The distribution mechanism is an open research question listed in [IDEA-9b26ba54](IDEA-9b26ba54).
...
Based on findings from [RES-8fee4dad](RES-8fee4dad), this framework guides where new features belong.
...
1. **Core graph protection** — [RULE-3c2da849](RULE-3c2da849) prevents plugins from modifying schemas, core knowledge, or the orchestrator prompt
...
- [RES-8fee4dad](RES-8fee4dad) — Multi-provider AI integration and plugin architecture research
- [IDEA-9b26ba54](IDEA-9b26ba54) — Component Library SDK for Plugin Views
- [IDEA-5113eeae](IDEA-5113eeae) — Plugin distribution model
- [IDEA-7a57ba89](IDEA-7a57ba89) — Integration ecosystem
- [RULE-3c2da849](RULE-3c2da849) — Core graph firmware protection
- [RULE-e1f1afc1](RULE-e1f1afc1) — Path-based knowledge injection
- [RULE-dd5b69e6](RULE-dd5b69e6) — Knowledge enforcement and tier model
