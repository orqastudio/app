...
The platform defines core artifact types (idea, decision, rule, lesson, knowledge, agent, doc, pillar, persona, vision, pivot, bug) in `libs/types/src/platform/core.json`. Plugins add domain-specific types on top via `orqa-plugin.json`.
...
| `idPrefix` | ID pattern (e.g., `EPIC` → `EPIC-797972a7`) |
...
The software plugin (`plugins/software-kanban/`) defines 5 types: milestone, epic, task, research, wireframe. It ships 9 custom relationships (delivers, fulfils, depends-on, realises, produces, yields, reports, fixes, affects).

- **KNOW-a700e25a** teaches agents the software delivery lifecycle
- **DOC-4554ff3e** teaches humans the same concepts
