...
- **Action taken (OrqaStudio):** Created `process/definition-of-ready.md` and `process/definition-of-done.md` at project start. CLAUDE.md references both gates as non-negotiable. Orchestrator verifies DoR before delegation and DoD before marking complete.
...
- **Action taken (OrqaStudio):** Four-layer content governance model defined at project start in `process/content-governance.md`. Rules, agents, skills, and docs each have clear ownership. `agent-maintainer` has periodic audit responsibility. `code-reviewer` includes doc-layer compliance in reviews.
