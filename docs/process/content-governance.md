...
Forge uses six distinct layers for governance knowledge: documentation, agent instructions, skills, rules, hooks, and hookify. Each layer owns a specific type of content. Mixing them creates maintenance burden and drift -- when a standard changes in one place, stale copies in other layers remain undetected.
...
Hookify files are **real-time enforcement** — pattern-based blocks and warnings that fire the moment an agent edits a file or runs a bash command. Where hooks enforce at lifecycle boundaries (session start, stop), hookify enforces at the action level (every file write, every command execution).
