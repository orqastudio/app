...
1. Scan `.orqa/ideas/` to determine the next ID
...
3. Add a brief entry in `docs/product/roadmap.md` if the idea is significant enough for roadmap visibility
...
2. Create `EPIC-NNN.md` in `.orqa/epics/` with:
   - `status: draft`
...
4. Update `docs/product/roadmap.md` to include the new epic
...
1. Verify the epic meets the Definition of Ready (`docs/process/definition-of-ready.md`)
...
3. If a task needs its own tracking, graduate it to `TASK-NNN.md` in `.orqa/tasks/`
4. Commit regularly to the worktree branch
5. Capture implementation lessons in `.orqa/lessons/`
...
5. Log any new lessons in `.orqa/lessons/`
...
1. Scan `.orqa/tasks/` for the next ID
