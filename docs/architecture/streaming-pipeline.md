...
End-to-end design for streaming AI responses from the Claude API through the sidecar, Rust backend, and into the Svelte UI. Covers the NDJSON protocol, parsing, event routing, token buffering, persistence, error handling, backpressure, reconnection, and cancellation.
...
```
+------------------+     SSE      +------------------+    NDJSON     +------------------+
|   Claude API     | -----------> |  Bun Sidecar     | -----------> |  Rust Backend     |
|  (30-100ms/tok)  |   network    |  (Agent SDK)     |   stdout     |  (tauri-plugin-   |
|                  |              |  ~0.1-0.5ms hop  |   pipe       |   shell spawn)    |
+------------------+              +------------------+              +------------------+
                                                                           |
                                                                    Channel<T>
                                                                    (ordered, indexed)
...
The sidecar hop adds ~0.1-0.5ms per event, which is negligible compared to Claude's 30-100ms per token generation time. `Channel<T>` is Tauri's recommended streaming mechanism -- faster than `emit`/`listen` events, with ordered delivery and index-based sequencing (AD-009).
