...
The agent registry reads `.claude/agents/*.md` at startup and indexes available roles.
...
src-tauri/src/
  agents/
    ...
    registry.rs        -- Reads .claude/agents/*.md, builds role index
...
- `docs/architecture/decisions.md` — AD-010: Tool implementation as MCP (orqa_ tool namespace)
- `docs/architecture/enforcement.md` — Enforcement engine that sub-agent calls pass through
- `docs/architecture/tool-definitions.md` — Built-in tool definitions and approval matrix
- `docs/process/orchestration.md` — Orchestration model and task delegation workflow
