...
class CompletionsProviderImpl {
    ...
    async getCompletions(document, position, completionContext, cancellationToken) {
        ...
        if (svelteNode?.type === 'InlineComponent' &&
            ...
            const mightBeAtStartTagWhitespace = this.mightBeAtStartTagWhitespace(document, originalOffset);
            ...
            if (!tsDoc.getFullText().includes(text) && typeChecker) {
                ...
                for (const entry of storeCompletions?.entries || []) {
                    if (entry.symbol &&
                        ...
                        !(0, utils_2.isGeneratedSvelteComponentName)(entry.name)) {
                        const aliasedSymbol = entry.symbol.flags & typescript_1.default.SymbolFlags.Alias
                            ? typeChecker.getAliasedSymbol(entry.symbol)
                            : entry.symbol;
                        if (aliasedSymbol.flags & typescript_1.default.SymbolFlags.Variable) {
    ...
    getCustomElementCompletions(lang, lsContainer, document, tsDoc, position) {
        ...
        const tag = (0, documents_1.getNodeIfIsInTagName)(document.html, offset);
        if (!tag || !tag.tag) {
            return;
        }
        let tags = (0, utils_3.getCustomElementsTags)(lang, lsContainer, tsDoc).filter((t) => t.startsWith(tag.tag));
        ...
        return tags.map((tag) => ({
            ...
            textEdit: vscode_languageserver_1.TextEdit.replace((0, utils_2.cloneRange)(replacementRange), tag),
            commitCharacters: []
        }));
    }
    ...
    async resolveCompletion(document, completionItem, cancellationToken) {
        ...
        const { tsDoc, lang, userPreferences, lsContainer } = await this.lsAndTsDocResolver.getLSAndTSDoc(document);
        const filePath = tsDoc.filePath;
        ...
        if (comp.__is_custom_element) {
            const doc = (0, utils_3.getCustomElementsDocument)(lang, lsContainer, tsDoc, comp.name);
            if (doc) {
                completionItem.documentation = { value: doc, kind: vscode_languageserver_1.MarkupKind.Markdown };
            }
            return completionItem;
        }
...
function createIsValidCompletion(document, position, hasParserError) {
    ...
    const isNoSvelte2tsxCompletion = (value) => {
        ...
        return value.name !== svelte2tsx_1.internalHelpers.renderName && !value.name.startsWith('$$_');
    };
