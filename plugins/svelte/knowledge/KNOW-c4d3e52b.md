...
summary: "SvelteKit patterns reference covering load function types (server vs universal, when to use each, secret protection via $env/static/private), page props typing (PageProps, LayoutProps with $props() and ./$types), form actions error handling (fail() for validation preserving form state vs throw error() for unexpected errors vs throw redirect()), and SSR state isolation (dangers of module-level state and global $state, safe patterns using locals, setContext, and browser guards)."
status: active
created: 2026-03-20
updated: 2026-03-20
relationships:
