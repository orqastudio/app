...
The pre-commit hook (`plugins/githooks/hooks/pre-commit`) logs every validation failure to `tmp/precommit-violations.jsonl`. Each entry records:
...
This log is append-only and persists across sessions (it lives in `tmp/`, which is gitignored).
...
| `tmp/precommit-violations.jsonl` | Append-only violation log |
| `plugins/agile-governance/orqa-plugin.json` | Rule schema with demotion fields |

## Related Documents

- [KNOW-a4fd56bd](KNOW-a4fd56bd) — Agent-facing knowledge pair for this documentation page
- [KNOW-449b1e02](KNOW-449b1e02) — Artifact Status Management (covers status transitions including active/inactive)
