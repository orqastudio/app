...
1. Reads `tmp/precommit-violations.jsonl` for violations in the rule's enforcement domain
...
The pre-commit hook appends to `tmp/precommit-violations.jsonl` whenever a validation check fails. Each line is a JSON object:
