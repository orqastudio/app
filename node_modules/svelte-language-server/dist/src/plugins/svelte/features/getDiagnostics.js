 ...
 */
async function getDiagnostics(document, svelteDoc, settings, cancellationToken) {
 ...
 */
async function tryGetDiagnostics(document, svelteDoc, settings, cancellationToken) {
    ...
    try {
        ...
        return (res.warnings || [])
            ...
            const codeDescription = getCodeDescription(warning.code, 'https://svelte.dev/docs/svelte/compiler-warnings#');
            if (codeDescription) {
            ...
            .filter((diag) => isNoFalsePositive(diag, document, ignoreScriptWarnings, ignoreStyleWarnings, ignoreTemplateWarnings));
    }
    catch (err) {
...
function createParserErrorDiagnostic(error, document) {
    ...
    if (diagnostic.message.includes('expected')) {
        ...
        if ((!document.config?.preprocess || document.config.isFallbackConfig) &&
            ...
            logger_1.Logger.error(`Parsing ${document.getFilePath()} failed. No preprocess config found but lang tag exists. Skip showing error because they likely use other preprocessors.`);
            return [];
        }
    ...
    const codeDescription = getCodeDescription(error.code, 'https://svelte.dev/docs/svelte/compiler-errors#');
    if (codeDescription) {
        diagnostic.codeDescription = codeDescription;
    }
    return [diagnostic];
}
function getCodeDescription(code, url) {
...
function isLowerLetter(charCode) {
...
function isNoFalsePositive(diag, doc, ignoreScriptWarnings, ignoreStyleWarnings, ignoreTemplateWarnings) {
    if ((ignoreTemplateWarnings || ignoreScriptWarnings) &&
        (typeof diag.code !== 'string' || !diag.code.startsWith('a11y'))) {
        return false;
    }
    if (ignoreStyleWarnings &&
...
function adjustMappings(diag, doc) {
    ...
    if ((diag.code === 'css-unused-selector' || diag.code === 'css_unused_selector') &&
        doc.styleInfo &&
