...
describe("ConversationStore", () => {
	...
	describe("setDefaultModel", () => {
		it("updates selectedModel and persists across clear()", () => {
			conversationStore.setDefaultModel("claude-opus-4-6");
			expect(conversationStore.selectedModel).toBe("claude-opus-4-6");
	...
	describe("clear", () => {
		it("resets all state to defaults", () => {
			// Explicitly set the default model so this test is independent of other test files
			conversationStore.setDefaultModel("auto");
			...
			expect(conversationStore.isLoading).toBe(false);
			expect(conversationStore.error).toBeNull();
			expect(conversationStore.selectedModel).toBe("auto"); // default fallback
			expect(conversationStore.pendingApproval).toBeNull();
			expect(conversationStore.processViolations).toEqual([]);
			expect(conversationStore.contextEntries).toEqual([]);
