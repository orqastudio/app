...
///
/// Returns an error if the event type or action string is not a recognized variant.
/// Unknown values are rejected rather than silently defaulted to prevent misconfigured
/// rules from being silently ignored.
fn parse_entry(raw: RawEntry) -> Result<EnforcementEntry, EngineError> {
