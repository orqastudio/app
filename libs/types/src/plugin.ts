...
// ---------------------------------------------------------------------------
// Content, Dependencies, Build & Lifecycle
// ---------------------------------------------------------------------------

/** Content mapping: plugin-local directory → .orqa/ destination. */
export interface PluginContentMapping {
	/** Plugin-local source directory (relative to plugin root). */
	source: string;
...
export interface PluginDependencies {
	/** npm packages to install in the plugin directory. */
	npm?: string[];
	/** System binaries required (checked but not installed). */
	system?: SystemRequirement[];
}

/** Plugin lifecycle hooks (custom commands run during install/uninstall). */
export interface PluginLifecycle {
	/** Command to run after content copy (cwd = plugin directory). */
	install?: string;
	/** Command to run before content removal (cwd = plugin directory). */
	uninstall?: string;
}
...
export interface PluginManifest {
	 ...
	 * Valid values: "methodology", "workflow", "domain-knowledge", "connector",
	 * "infrastructure", "app-extension", "sidecar".
	...
	/**
	 * Content artifact directories this plugin installs into the project.
	 * Keys are human-readable labels; values declare source and target paths.
	 */
	content?: Record<string, PluginContentMapping>;
...
/** A bundled knowledge artifact reference inside a plugin manifest. */
