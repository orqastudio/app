 ...
 */
export function generateOnDemandPreamble(
	onDemandCount: number,
): string {
	if (onDemandCount === 0) return "";
 ...
 */
export function queryOnDemandEntries(
	registry: PromptRegistry,
	options: KnowledgeQueryOptions,
): RegistryKnowledgeEntry[] {
	return registry.knowledge.filter((entry) => {
		if (entry.tier !== "on-demand") return false;
		...
		if (options.role) {
			const roleMatch =
				entry.roles.length === 0 || entry.roles.includes(options.role);
 ...
 */
function readKnowledgeArtifact(
	filePath: string,
	options: KnowledgeQueryOptions,
	...
	if (fm.type !== "knowledge") return null;
	...
	if (options.tags && options.tags.length > 0) {
		...
		const hasMatch = options.tags.some(
			(t) =>
				allTags.includes(t) ||
				title.toLowerCase().includes(t.toLowerCase()),
		);
		if (!hasMatch) return false as unknown as null;
	}
 ...
 */
export function countOnDemandEntries(registry: PromptRegistry): number {
