 ...
 * @returns Absolute path to the SDK cli.js binary.
 */
 ...
 * @param name - Raw tool name, possibly prefixed with "mcp__<server>__".
 * @returns The tool name without the MCP server prefix.
 */
 ...
 * @param model - Requested model identifier, or null/undefined for auto.
 ...
 * @param error - The thrown error value to classify.
 * @returns Classification with code, message, and recoverable flag.
 */
 ...
 * @param message - Raw message object from the Agent SDK iterator.
 * @param sendResponse - Callback to emit SidecarResponse events.
 */
...
export class ClaudeAgentProvider implements Provider {
     ...
     * @param send - Callback to emit the health_ok response.
     */
     ...
     * @param sessionId - OrqaStudio session ID for this conversation.
     * @param content - User message text to send.
     * @param model - Model identifier, or null for auto-selection.
     * @param systemPrompt - Additional system prompt text to prepend.
     * @param send - Callback to emit SidecarResponse events.
     * @param providerSessionId - Existing provider session UUID to resume, or null to start fresh.
     * @param enableThinking - Whether to enable extended thinking mode.
     */
    async streamMessage(
        sessionId: number,
        content: string,
            ...
            for await (const message of conversation) {
                ...
                if (message && typeof message === 'object') {
                    ...
                    if (msg.type === 'assistant' && msg.message && typeof msg.message === 'object') {
                        ...
                        translateAgentMessage(inner, send);
     ...
     * @param sessionId - OrqaStudio session ID whose stream to cancel.
     * @param send - Callback to emit the stream_cancelled event.
     */
     ...
     * @param sessionId - OrqaStudio session ID to include in the result event.
     * @param messages - Conversation messages to summarize.
     * @param send - Callback to emit the summary_result event.
     */
     ...
     * @param result - The tool result payload received from Rust.
     */
     ...
     * @param result - The tool approval payload received from Rust.
     */
     ...
     * @param toolCallId - The tool call ID to wait for.
     * @returns Promise that resolves with the tool result from Rust.
     */
     ...
     * with a matching tool_call_id.
     * @param toolCallId - The tool call ID to wait for.
     * @returns Promise that resolves with the approval decision from Rust.
     */
     ...
     * @param send - Callback to emit tool_execute and tool events.
     ...
     * @param toolName - Short name of the tool to execute.
     * @param args - Tool arguments as a plain object.
     * @param send - Callback to emit tool_execute and tracking events.
     * @returns MCP-compatible content array with the tool output.
     */
