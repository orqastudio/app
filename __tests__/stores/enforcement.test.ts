...
describe("EnforcementStore", () => {
	...
	describe("loadRules", () => {
		it("loads rules from backend", async () => {
			const mockRules: EnforcementRule[] = [
				{ name: "test-rule", scope: "system", entries: [{ event: "File", action: "Block", conditions: [{ field: "path", pattern: "src/**" }], pattern: null }], prose: "blocked" },
			];
	...
	describe("reloadRules", () => {
		it("calls reload then list", async () => {
			const mockRules: EnforcementRule[] = [
				{ name: "test-rule", scope: "system", entries: [{ event: "File", action: "Warn", conditions: [], pattern: null }], prose: "warning" },
			];
			mockInvoke
	...
	describe("violations", () => {
		it("addViolation appends to the list", () => {
			const v: EnforcementViolation = { rule_name: "test-rule", action: "Block", tool_name: "Write", detail: "blocked", timestamp: "2024-01-01" };
		...
		it("clearViolations resets the list", () => {
			enforcementStore.addViolation({ rule_name: "r1", action: "Block", tool_name: "Write", detail: "x", timestamp: "2024-01-01" });
			enforcementStore.addViolation({ rule_name: "r2", action: "Warn", tool_name: "Edit", detail: "y", timestamp: "2024-01-01" });
		...
		it("blockCount counts Block violations", () => {
			enforcementStore.violations = [
				{ rule_name: "r1", action: "Block", tool_name: "Write", detail: "x", timestamp: "2024-01-01" },
				{ rule_name: "r2", action: "Warn", tool_name: "Edit", detail: "y", timestamp: "2024-01-01" },
				{ rule_name: "r3", action: "Block", tool_name: "Write", detail: "z", timestamp: "2024-01-01" },
			];
