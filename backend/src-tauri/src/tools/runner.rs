//! Tool Runner — generic one-shot process executor for plugin-registered tools.
//!
//! Discovers registered tools from `plugin-tools.json` (written by the frontend
//! when plugins with `provides.tools` are loaded), spawns them as child
...
/// A tool registration loaded from plugin-tools.json.
...
/// Snapshot of the last run result for each tool.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Config file written by the frontend/SDK when tool plugins are registered.
const TOOLS_CONFIG_FILENAME: &str = "plugin-tools.json";

/// Read the tool registry from the config file at the project root.
fn read_tool_registry(project_root: &Path) -> Vec<RegisteredTool> {
    let config_path = project_root.join(TOOLS_CONFIG_FILENAME);
...
/// Manages one-shot tool execution and result caching.
pub struct ToolRunner {
    /// Cached last-run results, keyed by `"plugin:tool_key"`.
    last_results: Mutex<HashMap<String, ToolResult>>,
}
...
impl ToolRunner {
    ...
    /// Get all registered tools from the config file.
    pub fn registered_tools(&self, project_root: &Path) -> Vec<RegisteredTool> {
        read_tool_registry(project_root)
    }

    /// Run a specific tool and capture its output.
    ///
    ...
    pub fn statuses(&self, project_root: &Path) -> Vec<ToolStatus> {
        let tools = read_tool_registry(project_root);
...
mod tests {
    ...
    #[test]
    fn registered_tools_returns_empty_for_missing_config() {
        let runner = ToolRunner::new();
        let tools = runner.registered_tools(&PathBuf::from("/nonexistent"));
        assert!(tools.is_empty());
    }

    #[test]
    fn registered_tool_deserialization() {
        let json = r#"[{
        ...
        let tools: Vec<RegisteredTool> = serde_json::from_str(json).unwrap();
