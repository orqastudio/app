// Navigation tree scanner for artifact directories.
//
// Builds `NavTree` from a project's `.orqa/project.json` `artifacts` configuration.
// Each `ArtifactEntry` in the config drives the scan — no folder-guessing. The tree
// structure (groups -> types -> nodes) mirrors the sidebar hierarchy in the UI.
//
// This module is part of orqa-engine so all access layers (app, daemon, CLI) can
// produce the navigation tree without duplicating logic.
...
fn scan_type_nodes(
    ...
    type_key: &str,
    type_path: &str,
) -> Result<Vec<DocNode>, EngineError> {
    if type_key == "hooks" {
...
fn scan_recursive_nodes(dir: &Path, current_path: &str) -> Result<Vec<DocNode>, EngineError> {
...
fn build_dir_node(
    ...
    name: &str,
    current_path: &str,
) -> Result<Option<DocNode>, EngineError> {
...
///
/// Strips any file extension, replaces hyphens with spaces, and title-cases each word.
/// Preserves fully uppercase names (e.g. README, CHANGELOG, IDEA-001, EPIC-023).
...
mod tests {
    ...
    fn type_entry_with_md_files_produces_nodes() {
        let tmp = make_temp_project();
        let type_dir = tmp.path().join(".orqa/implementation/ideas");
    ...
    fn group_entry_produces_multi_type_group() {
        ...
        let ideas_dir = tmp.path().join(".orqa/implementation/ideas");
        ...
        let delivery_dir = tmp.path().join(".orqa/implementation");
        fs::write(
