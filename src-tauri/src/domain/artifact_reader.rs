...
use crate::domain::artifact::{DocNode, NavGroup, NavTree, NavType};
...
fn scan_type_nodes(type_dir: &Path, type_key: &str, type_path: &str) -> Result<Vec<DocNode>, OrqaError> {
    if type_key == "skills" {
...
fn scan_hooks_nodes(type_dir: &Path, type_path: &str) -> Result<Vec<DocNode>, OrqaError> {
    ...
    for entry in entries {
        ...
        let (label, status, description) = if is_md {
...
mod tests {
    ...
    fn recursive_scan_builds_tree_for_subdirectories() {
        ...
        // Nested subdir file.
        fs::write(nested_dir.join("deep-doc.md"), "---\ntitle: Deep Doc\n---\n").expect("deep");
        ...
        let tree = artifact_scan_tree(tmp.path(), &entries).expect("scan");
        assert_eq!(tree.groups.len(), 1, "documentation group should be present");

        let group = &tree.groups[0];
        let nav_type = &group.types[0];

        // Top-level nodes should be directory nodes (architecture, product) — no files at root.
        assert_eq!(nav_type.nodes.len(), 2, "architecture and product dirs, empty dir omitted");

        // Directories sorted alphabetically: architecture before product.
        assert_eq!(nav_type.nodes[0].label, "Architecture");
        assert!(nav_type.nodes[0].path.is_none(), "directory node has no path");
        ...
        assert_eq!(product_children[0].label, "Deep", "directory node for 'deep' subdir");
