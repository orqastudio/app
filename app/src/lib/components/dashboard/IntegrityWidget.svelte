			...
			{:else}
				...
				<HStack gap={3} justify="between">
					<SelectMenu
						...
						triggerLabel={categoryFilter === "all"
							? "All categories"
							: categoryLabels[categoryFilter as IntegrityCategory]}
						triggerSize="sm"
						align="start"
					/>
					...
					<HStack gap={1}>
						...
						<Button
							...
							size="sm"
							onclick={() => (severityFilter = "Info")}>Info</Button
						>
					</HStack>
				</HStack>
						...
						<TableHeader>
							<TableRow>
								...
								<TableHead
									...
									onclick={() => toggleSort("category")}>Category</TableHead
								>
								<TableHead
									sortable
									sorted={sortColumn === "artifact" ? (sortAsc ? "asc" : "desc") : false}
									onclick={() => toggleSort("artifact")}>Artifact</TableHead
								>
								<TableHead
									sortable
									sorted={sortColumn === "message" ? (sortAsc ? "asc" : "desc") : false}
									onclick={() => toggleSort("message")}>Message</TableHead
								>
							</TableRow>
						</TableHeader>
						<TableBody>
