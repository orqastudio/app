...
fn main() {
    ...
    if args.iter().any(|a| a == "--mcp") {
        ...
        if let Err(e) = orqa_studio_lib::servers::mcp::run(&project_path, resolve_daemon_port()) {
    ...
    if args.iter().any(|a| a == "--lsp") {
        ...
        if let Err(e) = orqa_studio_lib::servers::lsp::run(&project_path, resolve_daemon_port()) {
            // eprintln! is necessary here — tracing is not yet initialized, and
            // this is a fatal startup error that must reach the terminal.
            #[allow(clippy::print_stderr)]
            {
